...
module ut_gtm_network
   ...
   !> so that all values in common_variables are filled.
    subroutine test_gtm_network()
        ...
        call interp_network(4, 3, hydro_time_index)
        t=1
        do icell=1,npartition_x
            write(debug_unit,'(6f15.6)') flow_tmp(icell,t),flow_lo_tmp(icell,t), flow_hi_tmp(icell,t), &
                                area_tmp(icell,t),area_lo_tmp(icell,t), area_hi_tmp(icell,t)
        end do
        do i = 1, n_segm
