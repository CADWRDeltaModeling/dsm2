...
real(gtm_real),parameter :: x0 = 10000.0d0            !< Location of the initial condition discontinuity
real(gtm_real),parameter :: x_left = 10000.0d0        !< Left hand side of the channel zoppou tese
real(gtm_real),parameter :: x_right = 15000.0d0       !< Right hand side of the channel zoppou test
real(gtm_real),parameter :: x_l_time = zero           !< Left hand side of the channel time dependent test
real(gtm_real),parameter :: x_r_time = nine*two       !< Right hand side of the channel time dependent test
real(gtm_real),parameter :: start_time = 3000.0d0     !< Starts at 100000 sec (second) zoppou test
real(gtm_real),parameter :: end_time = 5000.0d0       !< Ends at 190000 (second)zoppou test
real(gtm_real),parameter :: start_t_time = 0.1d0      !< Starts at 0.1 sec (second)time dependent test
real(gtm_real),parameter :: end_t_time = three        !< Ends at 3 (second)time dependent test
real(gtm_real),parameter :: a0 = 1.0d7                !< Constant of area A=A0*(x^-1)
real(gtm_real),parameter :: a0_t = 1.6d0              !< Constant of area time dependent flow
real(gtm_real),parameter :: c0 = one                  !< Constant concentration
real(gtm_real),parameter :: d0 = 1.0d-6               !< Constant of dispersion coefficent D=D0*(x^2)
real(gtm_real),parameter :: u0 = 1.0d-4               !< Constant of velocity U=u0*x
real(gtm_real),parameter :: d0_t = 1.5d0              !< Constant of dispersion coefficent for time dependent test
real(gtm_real),parameter :: u0_t = 1.2d0              !< Constant of velocity for time dependent test
real(gtm_real),parameter :: omega = 12.41d0           !< Omega in velocity and diffusivity of time dependent test
...
if (cfl_number > one) then
   call gtm_fatal('Courant Number Larger Than One, Zoppou Test!') 
end if
! remove ">"
...
real(gtm_real),intent(out):: value_zoppou   !< Dirichlet initial condition at left side of channel
...
real(gtm_real),intent(out):: bc_value_zoppou(nconc)     !< Dirichlet initial condition at left side of channel
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!> Tests the convergence of error rate in advection and dispersion of 
!> mass with time varing velocity, dispersion coefficent, and constant area 
subroutine test_advection_diffusion_t_dependent(verbose)
...
total_time = end_t_time - start_t_time
                                      
cfl_number = three*u0_t*total_time*dble(nx_base)/dble(nstep_base)/test_domain_length
if (cfl_number > 0.99d0) then
   call gtm_fatal('Courant Number Larger Than One, Time Dependent Test!') 
end if
! remove ">"
!> load the initial values and reference final values to feed the test routine
call initial_fine_solution_time(fine_initial_condition, &
                                fine_solution,          &
                                nx_base,                &
                                nstep_base,             &
                                nconc)                  
...
call set_single_channel_boundary(dirichlet_advective_flux_lo, bc_data_time, &
                                 dirichlet_advective_flux_hi, bc_data_time, &
                                 dirichlet_diffusive_flux_lo, bc_data_time, &
                                 dirichlet_diffusive_flux_hi, extrapolate_hi_boundary_data)   !extrapolate_hi_boundary_data )
                                 !! note todo: Eli I changed it from extrapolation to real value (Kaveh)
...
subroutine time_solution(value_time_dep, &
                         xpos,           &
                         time)                
...
real(gtm_real),intent(out):: value_time_dep !< Dirichlet initial condition at left side of channel
real(gtm_real),intent(in) :: xpos           !< Location where data is requested
real(gtm_real),intent(in) :: time           !< Time
...
!> The cell averaging is done by the Composite Simpson's rule 
!> int (f,a,b) = 1/12 *(Fa+ 4*F2 + 2*F3 + 4*F4 + Fb)
subroutine initial_fine_solution_time(fine_initial_condition, &
                                      fine_solution,          &
                                      nx_base,                &
                                      nstep_base,             &
                                      nconc)
...
dx = (x_r_time -  x_l_time)/dble(nx_base)
fine_solution = zero
fine_initial_condition = zero

do icell=1,nx_base
  ! x = x0
  xpos    = x_l_time +(dble(icell)-one)*dx
  call time_solution(point_value,xpos,start_t_time)
  fine_initial_condition(icell,:) = fine_initial_condition(icell,:) + (half/six)*point_value
  call time_solution(point_value,xpos,end_t_time)
  fine_solution(icell,:) = fine_solution(icell,:) + (half/six)*point_value

  ! x = x0 + 1/4L
  xpos    = x_l_time +(dble(icell)- three/four)*dx
  call time_solution(point_value,xpos,start_t_time)
  fine_initial_condition(icell,:) = fine_initial_condition(icell,:) + (two/six)*point_value
  call time_solution(point_value,xpos,end_t_time)
  fine_solution(icell,:) = fine_solution(icell,:) + (two/six)*point_value
  ...
  ! x = x0 + 2/4L
  xpos    = x_l_time +(dble(icell)- half)*dx
  call time_solution(point_value,xpos,start_t_time)
  fine_initial_condition(icell,:) = fine_initial_condition(icell,:) + (one/six)*point_value
  call time_solution(point_value,xpos,end_t_time)
  fine_solution(icell,:) = fine_solution(icell,:) + (one/six)*point_value
  ...
  ! x = x0 + 3/4L
  xpos    = x_l_time +(dble(icell)- fourth)*dx
  call time_solution(point_value,xpos,start_t_time)
  fine_initial_condition(icell,:) = fine_initial_condition(icell,:) + (two/six)*point_value
  call time_solution(point_value,xpos,end_t_time)
  fine_solution(icell,:) = fine_solution(icell,:) + (two/six)*point_value
