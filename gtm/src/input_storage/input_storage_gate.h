#ifndef gate_STORAGE_H__
#define gate_STORAGE_H__
/**
...
/** Structure representing input of type gate.
...
public:
  ...
  /** Data type gate, default constructor */  
  ...
  /** Construct a gate with actual data values */
  gate(const  char a_name[32],const  char a_from_obj[16],const  char a_from_identifier[32],const int & a_to_node, bool a_used=true, int a_layer = 0) :
    to_node(a_to_node),
    used(a_used),
    layer(a_layer)
  {
  ...
  */
  ...
  {
     return gate::identifier_type( name );
  }
  ...
  gate& operator=(const gate& rhs)
  {
    ...
    this->to_node=rhs.to_node;
    used = rhs.used;
    layer = rhs.layer;
    return *this;
  }

  /** Return the class name of this object (gate) */
...
typedef HDFTableManager<gate> gate_table;
...
*/  
FCALL void gate_clear_buffer_f();

/** query number of records being stored in buffer */
FCALL int gate_buffer_size_f();


/** append to buffer, compatible with fortran, returns new size*/
FCALL void gate_append_to_buffer_f(const  char a_name[32],const  char a_from_obj[16],const  char a_from_identifier[32],const int * a_to_node, int * ierror, 
              const int name_len,const int from_obj_len,const int from_identifier_len);
  
/** both makes the table and writes the contents of the buffer to it */
FCALL void gate_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror);

/** reads the table in from a file into the buffer*/
FCALL void gate_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror);

/** query size information about the table in hdf5
*/
FCALL void gate_number_rows_hdf5_f(const hid_t* file_id, hsize_t* nrecords, int* ierror);


/** get one row worth of information from the buffer */
FCALL void gate_query_from_buffer_f(size_t* row, 
...
/**
  ...
  */
FCALL void gate_prioritize_buffer_f(int* ierror);
/**
 ...
 */
FCALL void gate_write_buffer_to_text_f(const char* file, const bool* append, int* ierror, int filelen);
