...
/**
  ...
  4. Clear the buffer.
*/
#include "input_storage_scalar.h"
...
istream& operator>> (istream& stream, scalar & obj)
{
  ...
  string tempstr;
   ...
   {
      cout << "fatal error" <<endl;
         throw logic_error("String too long (max width 64):" + (*beg));
   }
   ;
  return stream;
}
...
template<>
HDFTableManager<scalar>::HDFTableManager() :
    description(scalar_table_description()),  
    m_default_fill(scalar("","")){}

template<>
void HDFTableManager<scalar>::prioritize_buffer()
{

    // Sort by identifier (lexicographical order) and
    // layer (decreasing order of priority)
    std::sort(buffer().begin(),buffer().end());
    vector<scalar>::const_iterator dupl = adjacent_find(buffer().begin(),buffer().end());
    if ( dupl != buffer().end())
    {   
    ...
    buffer().erase(unique(buffer().begin(),buffer().end(),identifier_equal<scalar>()),buffer().end());
    // Eliminate items that are not used. This must be done after lower layers have been removed
    buffer().erase(remove_if(buffer().begin(), buffer().end(),not1(entry_used<scalar>())), buffer().end());
    
}

TableDescription scalar_table_description(){
  const char* title = "scalar";
  const size_t size = sizeof(scalar);
  const size_t nfields = 2;
  scalar default_struct = scalar("","");
  const char* fnames[] =  {"name","value"};
  const hid_t ftypes[] =  {
            string_type(32),string_type(64)
               };
...
/**
  Clear the storage buffer for objects of type scalar
*/  
void scalar_clear_buffer_f(){
  //scalar_table::instance().buffer().destroy();
  scalar_table::instance().buffer().clear();
}

/** append to buffer, compatible with fortran, returns new size*/
void scalar_append_to_buffer_f(const  char a_name[32],const  char a_value[64], int * ierror, 
              const int name_len,const int value_len)
{
 _TRAP_EXCEPT(*ierror,
   scalar_table::instance().buffer().push_back(
                                      scalar(
                                      a_name,a_value
                                      ));
...
void scalar_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  scalar_table & table = scalar_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( scalar_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //scalar_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void scalar_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    scalar_table & table = scalar_table::instance();
...
void scalar_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     scalar_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
void scalar_query_from_buffer_f(size_t* row, 
                         char a_name[32], char a_value[64], int * ierror, 
              int name_len,int value_len
                        )
{
 _TRAP_EXCEPT(*ierror,
    //if (row > scalar_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    scalar obj =scalar_table::instance().buffer()[ndx];
    memcpy(a_name,obj.name,32);
    memcpy(a_value,obj.value,64);
    if (strlen(a_name) < 32)fill(a_name+strlen(a_name),a_name+32,' ');
    if (strlen(a_value) < 64)fill(a_value+strlen(a_value),a_value+64,' ');
...
void scalar_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  scalar_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type scalar */
int scalar_buffer_size_f()
{ 
  return (int) scalar_table::instance().buffer().size();
}

void scalar_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("scalar");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<scalar> & obs = scalar_table::instance().buffer();
   scalar_table& table = scalar_table::instance();
   ...
   out << endl;
   for (vector<scalar>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const scalar & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void scalar_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  scalar_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
