...
/**
  ...
  4. Clear the buffer.
*/
#include "input_storage_gate_weir_device.h"
...
ostream& operator<<(ostream & stream, const gate_weir_device & obj)
{  
...
istream& operator>> (istream& stream, gate_weir_device & obj)
{
  string str;
  getline(stream,str);
  ...
  string tempstr;
   ...
   else
   {
      cout << "fatal error" <<endl;
        ...
        obj.height = strtod((beg++)->c_str(),NULL);
...
template<>
HDFTableManager<gate_weir_device>::HDFTableManager() :
    description(gate_weir_device_table_description()),  
    m_default_fill(gate_weir_device("","",-901,-901.0,-901.0,-901.0,-901.0,-901.0,"")){}

template<>
void HDFTableManager<gate_weir_device>::prioritize_buffer()
{
    ...
    buffer().erase(remove_if(buffer().begin(),
                           buffer().end(),
                           not1(mem_fun_ref(&gate_weir_device::parent_valid))),buffer().end());
    
}

TableDescription gate_weir_device_table_description(){
  const char* title = "gate_weir_device";
  const size_t size = sizeof(gate_weir_device);
  const size_t nfields = 9;
  gate_weir_device default_struct = gate_weir_device("","",-901,-901.0,-901.0,-901.0,-901.0,-901.0,"");
  const char* fnames[] =  {"gate_name","device","nduplicate","width","elev","height","cf_from_node","cf_to_node","default_op"};
  const hid_t ftypes[] =  {
            string_type(32),string_type(32),H5T_NATIVE_INT,H5T_NATIVE_DOUBLE,H5T_NATIVE_DOUBLE,H5T_NATIVE_DOUBLE,H5T_NATIVE_DOUBLE,H5T_NATIVE_DOUBLE,string_type(16)
               };
  ...
  const size_t foffsets[] ={
             ...
             ((char*)&default_struct.height - (char*)&default_struct),
...
/**
  Clear the storage buffer for objects of type gate_weir_device
*/  
void gate_weir_device_clear_buffer_f(){
  //gate_weir_device_table::instance().buffer().destroy();
  gate_weir_device_table::instance().buffer().clear();
}

/** append to buffer, compatible with fortran, returns new size*/
void gate_weir_device_append_to_buffer_f(const  char a_gate_name[32],const  char a_device[32],const int * a_nduplicate,const double * a_width,const double * a_elev,const double * a_height,const double * a_cf_from_node,const double * a_cf_to_node,const  char a_default_op[16], int * ierror, 
              const int gate_name_len,const int device_len,const int default_op_len)
{
 _TRAP_EXCEPT(*ierror,
   gate_weir_device_table::instance().buffer().push_back(
                                      gate_weir_device(
                                      a_gate_name,a_device,*a_nduplicate,*a_width,*a_elev,*a_height,*a_cf_from_node,*a_cf_to_node,a_default_op
                                      ));
 ) // end of exception trap
}
  
/** both makes the table and writes the contents of the buffer to it */
void gate_weir_device_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  gate_weir_device_table & table = gate_weir_device_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( gate_weir_device_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //gate_weir_device_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void gate_weir_device_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    gate_weir_device_table & table = gate_weir_device_table::instance();
...
void gate_weir_device_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     gate_weir_device_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
void gate_weir_device_query_from_buffer_f(size_t* row, 
                         char a_gate_name[32], char a_device[32],int * a_nduplicate,double * a_width,double * a_elev,double * a_height,double * a_cf_from_node,double * a_cf_to_node, char a_default_op[16], int * ierror, 
              int gate_name_len,int device_len,int default_op_len
                        )
{
 _TRAP_EXCEPT(*ierror,
    //if (row > gate_weir_device_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    gate_weir_device obj =gate_weir_device_table::instance().buffer()[ndx];
...
void gate_weir_device_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  gate_weir_device_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type gate_weir_device */
int gate_weir_device_buffer_size_f()
{ 
  return (int) gate_weir_device_table::instance().buffer().size();
}

void gate_weir_device_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("gate_weir_device");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<gate_weir_device> & obs = gate_weir_device_table::instance().buffer();
   gate_weir_device_table& table = gate_weir_device_table::instance();
   ...
   out << endl;
   for (vector<gate_weir_device>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const gate_weir_device & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void gate_weir_device_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  gate_weir_device_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
