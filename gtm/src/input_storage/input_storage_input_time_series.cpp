...
template<>
HDFTableManager<input_time_series>::HDFTableManager() :
    description(input_time_series_table_description()),  
    m_default_fill(input_time_series("","","","","")){}

template<>
void HDFTableManager<input_time_series>::prioritize_buffer()
{

    // Sort by identifier (lexicographical order) and
    // layer (decreasing order of priority)
    std::sort(buffer().begin(),buffer().end());
    vector<input_time_series>::const_iterator dupl = adjacent_find(buffer().begin(),buffer().end());
    if ( dupl != buffer().end())
    {   
    ...
    buffer().erase(unique(buffer().begin(),buffer().end(),identifier_equal<input_time_series>()),buffer().end());
    // Eliminate items that are not used. This must be done after lower layers have been removed
    buffer().erase(remove_if(buffer().begin(), buffer().end(),not1(entry_used<input_time_series>())), buffer().end());
    
}

TableDescription input_time_series_table_description(){
  const char* title = "input_time_series";
  const size_t size = sizeof(input_time_series);
  const size_t nfields = 5;
  input_time_series default_struct = input_time_series("","","","","");
  const char* fnames[] =  {"group_name","variable","fillin","file","path"};
  const hid_t ftypes[] =  {
...
/**
  Clear the storage buffer for objects of type input_time_series
*/  
void input_time_series_clear_buffer_f(){
  //input_time_series_table::instance().buffer().destroy();
  input_time_series_table::instance().buffer().clear();
}

/** append to buffer, compatible with fortran, returns new size*/
void input_time_series_append_to_buffer_f(const  char a_group_name[32],const  char a_variable[16],const  char a_fillin[8],const  char a_file[128],const  char a_path[80], int * ierror, 
              const int group_name_len,const int variable_len,const int fillin_len,const int file_len,const int path_len)
{
 _TRAP_EXCEPT(*ierror,
   input_time_series_table::instance().buffer().push_back(
                                      input_time_series(
                                      a_group_name,a_variable,a_fillin,a_file,a_path
                                      ));
 ) // end of exception trap
}
  
/** both makes the table and writes the contents of the buffer to it */
void input_time_series_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  input_time_series_table & table = input_time_series_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( input_time_series_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //input_time_series_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void input_time_series_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    input_time_series_table & table = input_time_series_table::instance();
...
void input_time_series_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     input_time_series_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
void input_time_series_query_from_buffer_f(size_t* row, 
...
{
 _TRAP_EXCEPT(*ierror,
    //if (row > input_time_series_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    input_time_series obj =input_time_series_table::instance().buffer()[ndx];
...
void input_time_series_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  input_time_series_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type input_time_series */
int input_time_series_buffer_size_f()
{ 
  return (int) input_time_series_table::instance().buffer().size();
}

void input_time_series_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("input_time_series");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<input_time_series> & obs = input_time_series_table::instance().buffer();
   input_time_series_table& table = input_time_series_table::instance();
   ...
   out << endl;
   for (vector<input_time_series>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const input_time_series & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void input_time_series_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  input_time_series_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
