...
/**
  ...
  4. Clear the buffer.
*/
#include "input_storage_source_flow.h"
...
ostream& operator<<(ostream & stream, const source_flow & obj)
{  
...
istream& operator>> (istream& stream, source_flow & obj)
{
  string str;
  getline(stream,str);
...
template<>
HDFTableManager<source_flow>::HDFTableManager() :
    description(source_flow_table_description()),  
    m_default_fill(source_flow("",-901,-901,"","","")){}

template<>
void HDFTableManager<source_flow>::prioritize_buffer()
{

    // Sort by identifier (lexicographical order) and
    // layer (decreasing order of priority)
    std::sort(buffer().begin(),buffer().end());
    vector<source_flow>::const_iterator dupl = adjacent_find(buffer().begin(),buffer().end());
    if ( dupl != buffer().end())
    {   
    ...
    buffer().erase(unique(buffer().begin(),buffer().end(),identifier_equal<source_flow>()),buffer().end());
    // Eliminate items that are not used. This must be done after lower layers have been removed
    buffer().erase(remove_if(buffer().begin(), buffer().end(),not1(entry_used<source_flow>())), buffer().end());
    
}

TableDescription source_flow_table_description(){
  const char* title = "source_flow";
  const size_t size = sizeof(source_flow);
  const size_t nfields = 6;
  source_flow default_struct = source_flow("",-901,-901,"","","");
...
/**
  Clear the storage buffer for objects of type source_flow
*/  
void source_flow_clear_buffer_f(){
  //source_flow_table::instance().buffer().destroy();
  source_flow_table::instance().buffer().clear();
}

/** append to buffer, compatible with fortran, returns new size*/
void source_flow_append_to_buffer_f(const  char a_name[32],const int * a_node,const int * a_sign,const  char a_fillin[8],const  char a_file[128],const  char a_path[80], int * ierror, 
              const int name_len,const int fillin_len,const int file_len,const int path_len)
{
 _TRAP_EXCEPT(*ierror,
   source_flow_table::instance().buffer().push_back(
                                      source_flow(
...
void source_flow_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  source_flow_table & table = source_flow_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( source_flow_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //source_flow_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void source_flow_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    source_flow_table & table = source_flow_table::instance();
...
void source_flow_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     source_flow_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
void source_flow_query_from_buffer_f(size_t* row, 
...
{
 _TRAP_EXCEPT(*ierror,
    //if (row > source_flow_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    source_flow obj =source_flow_table::instance().buffer()[ndx];
...
void source_flow_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  source_flow_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type source_flow */
int source_flow_buffer_size_f()
{ 
  return (int) source_flow_table::instance().buffer().size();
}

void source_flow_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("source_flow");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<source_flow> & obs = source_flow_table::instance().buffer();
   source_flow_table& table = source_flow_table::instance();
   ...
   out << endl;
   for (vector<source_flow>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const source_flow & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void source_flow_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  source_flow_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
