...
/**
  ...
  4. Clear the buffer.
*/
#include "input_storage_input_gate.h"
...
template<>
HDFTableManager<input_gate>::HDFTableManager() :
    description(input_gate_table_description()),  
    m_default_fill(input_gate("","","","","","")){}

template<>
void HDFTableManager<input_gate>::prioritize_buffer()
{

    // Sort by identifier (lexicographical order) and
    // layer (decreasing order of priority)
    std::sort(buffer().begin(),buffer().end());
    vector<input_gate>::const_iterator dupl = adjacent_find(buffer().begin(),buffer().end());
    if ( dupl != buffer().end())
    {   
    ...
    buffer().erase(unique(buffer().begin(),buffer().end(),identifier_equal<input_gate>()),buffer().end());
    // Eliminate items that are not used. This must be done after lower layers have been removed
    buffer().erase(remove_if(buffer().begin(), buffer().end(),not1(entry_used<input_gate>())), buffer().end());
    
}

TableDescription input_gate_table_description(){
  const char* title = "input_gate";
  const size_t size = sizeof(input_gate);
  const size_t nfields = 6;
  input_gate default_struct = input_gate("","","","","","");
  const char* fnames[] =  {"gate_name","device","variable","fillin","file","path"};
  const hid_t ftypes[] =  {
...
/**
  Clear the storage buffer for objects of type input_gate
*/  
void input_gate_clear_buffer_f(){
  //input_gate_table::instance().buffer().destroy();
  input_gate_table::instance().buffer().clear();
}

/** append to buffer, compatible with fortran, returns new size*/
void input_gate_append_to_buffer_f(const  char a_gate_name[32],const  char a_device[32],const  char a_variable[16],const  char a_fillin[8],const  char a_file[128],const  char a_path[80], int * ierror, 
...
{
 _TRAP_EXCEPT(*ierror,
   input_gate_table::instance().buffer().push_back(
                                      input_gate(
                                      a_gate_name,a_device,a_variable,a_fillin,a_file,a_path
                                      ));
 ) // end of exception trap
}
  
/** both makes the table and writes the contents of the buffer to it */
void input_gate_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  input_gate_table & table = input_gate_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( input_gate_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //input_gate_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void input_gate_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    input_gate_table & table = input_gate_table::instance();
...
void input_gate_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     input_gate_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
void input_gate_query_from_buffer_f(size_t* row, 
...
{
 _TRAP_EXCEPT(*ierror,
    //if (row > input_gate_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    input_gate obj =input_gate_table::instance().buffer()[ndx];
...
void input_gate_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  input_gate_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type input_gate */
int input_gate_buffer_size_f()
{ 
  return (int) input_gate_table::instance().buffer().size();
}

void input_gate_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("input_gate");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<input_gate> & obs = input_gate_table::instance().buffer();
   input_gate_table& table = input_gate_table::instance();
   ...
   out << endl;
   for (vector<input_gate>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const input_gate & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void input_gate_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  input_gate_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
