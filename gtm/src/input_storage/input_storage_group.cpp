...
{  
  ...
  return stream <<  
            setw(max(4+1,(int)(4+strlen(obj.name))))
            << setfill(' ')
            << left
...
template<>
HDFTableManager<group>::HDFTableManager() :
    description(group_table_description()),  
    m_default_fill(group("")){}

template<>
void HDFTableManager<group>::prioritize_buffer()
{

    // Sort by identifier (lexicographical order) and
    // layer (decreasing order of priority)
    std::sort(buffer().begin(),buffer().end());
    vector<group>::const_iterator dupl = adjacent_find(buffer().begin(),buffer().end());
    if ( dupl != buffer().end())
    {   
    ...
    buffer().erase(unique(buffer().begin(),buffer().end(),identifier_equal<group>()),buffer().end());
    // Eliminate items that are not used. This must be done after lower layers have been removed
    buffer().erase(remove_if(buffer().begin(), buffer().end(),not1(entry_used<group>())), buffer().end());
    
}

TableDescription group_table_description(){
  const char* title = "group";
  const size_t size = sizeof(group);
  const size_t nfields = 1;
  group default_struct = group("");
  const char* fnames[] =  {"name"};
  const hid_t ftypes[] =  {
            string_type(32)
               };
...
/**
  Clear the storage buffer for objects of type group
*/  
void group_clear_buffer_f(){
  //group_table::instance().buffer().destroy();
  group_table::instance().buffer().clear();
}

/** append to buffer, compatible with fortran, returns new size*/
void group_append_to_buffer_f(const  char a_name[32], int * ierror, 
              const int name_len)
{
 _TRAP_EXCEPT(*ierror,
   group_table::instance().buffer().push_back(
                                      group(
                                      a_name
                                      ));
...
void group_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  group_table & table = group_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( group_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //group_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void group_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    group_table & table = group_table::instance();
...
void group_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     group_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
void group_query_from_buffer_f(size_t* row, 
                         char a_name[32], int * ierror, 
              int name_len
                        )
{
 _TRAP_EXCEPT(*ierror,
    //if (row > group_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    group obj =group_table::instance().buffer()[ndx];
...
void group_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  group_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type group */
int group_buffer_size_f()
{ 
  return (int) group_table::instance().buffer().size();
}

void group_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("group");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<group> & obs = group_table::instance().buffer();
   group_table& table = group_table::instance();
   ...
   out << endl;
   for (vector<group>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const group & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void group_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  group_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
