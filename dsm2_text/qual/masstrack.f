...
C-----COMMAND=1   Calculate the total (constituent) mass stored
C-----------------in all the channels and reservoirs.
      ...
      IF(COMMAND.EQ.2)THEN   ! Compu. the amount of const. entering and leaving the system
         do CONS_NO=1,neq
            IF(Jtime.EQ.1)THEN                                ! Initialize
               ...
               do qualnode=1,nnodes
                  dsmnode=nodequal2dsm(qualnode)
                  export_num=node_export_id(qualnode)
               enddo
            ENDIF
         enddo
  
c--------Clifton Court Hard-coded
...
c--------nodes
         do dsmnode=1,max_nodes
            qualnode=nodedsm2qual(dsmnode)
            if (qualnode .gt. 0 ) then
               if( upnode_flag_of_chan(dsmnode) 
     &              .or. downnode_flag_of_chan(dsmnode) ) then ! flag for the node is on
                  call node_rate(qualnode,TO_OBJ,QEXT_FLOWS,objflow,massrate)
                  do CONS_NO=1,neq
                     totconstitenter(CONS_NO) = totconstitenter(CONS_NO) 
     &                    + massrate(CONS_NO)*dtt
                  enddo
                  call node_rate(qualnode,FROM_OBJ,QEXT_FLOWS,objflow,massrate)
                  do CONS_NO=1,neq
                     export_num=node_export_id(qualnode)
     ...
     &                       - massrate(CONS_NO)*dtt
                  ...
                  do i=1,nstgbnd
                     if (dsmnode .eq. stgbnd(i).node) then
                        branch=listdown(qualnode,1)
     ...
     &                             gtrib(CONS_NO,nxsec(branch),branch)
         ...
         do CONS_NO=1,NEQ
c-----------Mass tracking for boundary lines
...
c--------------------boundary line at the upstream end of the channel
                     if(flow(n,1,1).ge.0.)then
c-----------------------flow in the positive direction
                        if(jncu(n).le.njnct)then
c--------------------------internal node. Use junction mixed value
                           mass_entry = flow(n,1,1) * dtt * cj(CONS_NO,jncu(n))
                        else
c--------------------------external node. Use first parcel info
...
c--------------------boundary at the downstream end of the channel
                     if(flow(n,1,nxsec(n)).ge.0.)then
c-----------------------flow in the positive direction
                        mass_entry=gptd(CONS_NO,n)
                     else
c-----------------------flow in the negative direction
                        if(jncd(n).le.njnct)then
c--------------------------internal node. Use junction mixed value
...
c--------------------user has specified the positive direction for
c--------------------mass-track boundary line for this channel to be
c--------------------the same as what the grid suggests.
c--------------------No need to change the sign
                  else
c--------------------need to change the sign. See above
...
c-----------The main mass tracking output for the Delta
c-----------Names are hard-coded
 ...
 966           format('pct_mass_',i2,'.out')
            ...
            write(iun,967) constituents(CONS_NO).constituent,constituents(CONS_NO).loc_name
 967        format('Constituent:',a20,'            Source: ',a20)
...
c-----------Mass-tracking output header (tidal day hard-coded)
 968        format(/
     &           'Tday    Delta      Islands      SWP         Chipps   '1x,3(2x,a8),'    Decay    ',
     &           '      Total')
c-----------initialize masstrack through boundary lines
            do i=1,num_masstrack_boundary_lines
               iun=100+i
               if(i.le.9)then
                  write(fle,961) i
...
c--------------roundoff error. Not a big deal
            else 
...
c------------------------------------------------print_results_for_masstracking
      ...
      DO CONS_NO=1,NEQ 
         ...
         IF(NET_MASS_ENTER.EQ.0.)THEN
            ...
            IF(IWARN(CONS_NO).EQ.0) WRITE(UNIT_ERROR,1210) constituents(CONS_NO).constituent, constituents(CONS_NO).loc_name
 1210       format(/'---------------------------------------------------------------'/
     &           'Warning... Mass-tracking is not possible for '/
     &           '           Constituent:',a20,'            Source: ',a20/
...
c--------This is hard-coded...
c--------Delta is defined using the last region, which includes
c--------all the channels and open water areas east of Chipps island,
c--------except Clifton Court Forebay itself.
...
c--------This is hard-coded
c--------Chipps Island is defined by Boundary Line#1
...
c-------- Hard code frquency of the output (Most likely will not change) (25hours)

         iprnt_mass=25*60   ! Frequency of output in minutes

c--------Assign three export locations as default values.
c--------These values will be over-written by input file, if any.
...
c--------If there are new locations in the input file, they will overwrite
c--------the dafault values set earlier.
 ...
 685     continue
         ...
         do i=1,num_export_nodes
c-----------Convert export nodes number into qual node numbers
            list_export_nodes(i)=nodedsm2qual(list_export_nodes(i))
c-----------Reverse translation (not sure about it, whether it is reverse
c-----------translations or not --ganesh).
            node_export_id(list_export_nodes(i))=i
         enddo

c--------Check to see if mass tracking by region is requested 
c--------(Basically see if the input file is present)
c--------Here are the assumptions in the input files:
c-------- 1- Each region has at least one channel
c-------- 2- Once a new region is declared, no new channels or reservoirs
c--------    are introduced for a previously defined region (Any region
c--------    defined is a new region.
c-------- 3- There can be no missing region number
c-------- 4- channel numbers are read with i3 format, and
c--------    reservoir numbers are read with i2 format (**FORMATED INPUT**)
...
c-----------reservoir number specified
 ...
 990     close(81)
         ...
         do CONS_NO=1,neq
c-----------mass-tracking by region output-filenames hard-coded
            iun=190+CONS_NO
            if(CONS_NO.le.9)then
...
c-----------header hard-coded with tidal day and 11 regions
...
c--------See if any Boundary lines defined
         num_masstrack_boundary_lines=0
c--------masstrack input file defining the boundary lines(filename hard-coded)
c-------- minimum of 1 boundary line is needed to define Chipps Island
c--------(Chipps Island should be  defined as the first boundary line (hard-coded)
...
c-------------------------------------------read_nodeflags_for_masstracking_region
...
c---------------------------------------update_resvol_for_masstracking_region
...
c-----The flags are set true if they are located on either end of the channel.
     ...
     &           reschgmass(i,CONS_NO)) / (RESVOL(I) + reschgvol(i))
         ...
         IF(RESVOL(I) .LT. 0.)THEN
            ERZ=RESVOL(I)/ARES(I)
            WRITE(UNIT_ERROR,922)I,ERZ
 922        FORMAT(' ERROR ... RESERVOIR: ',I3,' HAS NEGATIVE',
     &           ' DEPTH =', 1PE12.5)
            call exit(2)
         ENDIF         
      ENDDO
