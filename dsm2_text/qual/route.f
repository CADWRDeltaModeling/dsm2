...
C     + + + LOCAL VARIABLES + + +
     ...
     &     ND(NOPR),NDMAX,NSL
      REAL    COF,COF1,DF(NOPR),DMF(NOPR),DQ(NOPR),
     &     PRDT(NOPR),PVT,RATIO,RDT,C(MAX_CONSTITUENT)

      INTEGER KR, NN, NDD(NOPR+1)
      REAL    RQ,CONCMIX,DTSEC,CLOG2,RNDMAX
      REAL    DIFFGPT(NOPR+1)
      PARAMETER (CLOG2=0.3010300)
      INTEGER JN,NSN,NXSECN
      LOGICAL NEWPARCEL(NOPR)

      real objflow,massrate(max_constituent) ! object flow and massrates
...
C-----------parcel passed grid
     ...
     &              int2ext(N),' DRIED UP!'
               WRITE(UNIT_ERROR,*) '          TIME: ',current_dt
               call exit(2)
            ENDIF
            RDT=RDT-DTSUB
