...
C!    or see our home page: http://wwwdelmod.water.ca.gov/
      ...
      real cp(max_constituent),del_c,tol_c,delta_c
      logical converged, firsttime

c-----local variables---------------------------

      real c1(max_constituent),scsk1(max_constituent)
      real rirdt, idtm,  nsmalldtt

      integer i
      real dt_rem, dt_react
      firsttime = .true.
      if (irc. eq. 1) then
...
c-----write(iscr,*)'tol. not met',' with 2 repeat',' reduce step'
 78   if (chan_res. eq. 2) go to 200 ! reservoir kinetics
      rirdt=ifix(dt_rem*100.0+0.5)
      idtm=ifix(dt_react*100.0+0.5)
