...
c-----calculate source, sink terms for DO
      ...
      real  c(max_constituent)
      real k2, k2min, k2max,tw, p, pwv, twk, osf,oss, temp, reaer, os, phi
      real twksq,twk4,tempval, kwind
            ...
            if (mod(julmin, 120). eq. 0) then
               if(chan_res.eq.1) then
                  if(int2ext(n). eq. 20) then
                     write(idoout,999)current_dt,int2ext(n),osf,oss,temp, os, c(mdo),
     &                    reaer,vel,k2min,benth,kwind,dqq(int2ext(n)),c(mec)
                  endif
               elseif(chan_res.eq.2) then
                  write(irdoout,998)current_dt,irev,osf,oss,temp, os,
...
c-----calculate source, sink terms for ALGAE
      ...
      real c(max_constituent),xn, fn, fp, lambda, grw, fl, resp, factor, mort
      real cmalg
      ...
      if(print_level.ge.3)then
         if(iskip. eq. 1)then
            if (mod(julmin, 120). eq. 0) then
               if (chan_res. eq. 1) then
                  if(int2ext(n). eq. 20) then
                     write(ialgout,999)current_dt,int2ext(n),c(mdo),cmalg,c(mbod),
     &                    c(mnh3),c(mno3),c(mpo4),c(mtemp),grw,tsolhr,fl,fn,fp,mort
                  endif
               elseif (chan_res. eq. 2)then
                  write(iralgout,998)current_dt,irev,c(mdo),cmalg,c(mbod),
...
c-----calculated in subroutine heat
      ...
      real c(max_constituent)
      real tw, vpw, evapor, hb, hc, he, hsnet,evap_rate
      tw = c(mtemp)*1.8 + 32
...
C-----Compute net heat flux from all sources
      ...
      hsnet = tsolhr + (ha + hc - hb - he)*float(time_step)/60.
 
C-----Contribution to SCSK is net radiation/(depth*density*heat cap)

      scsk(mtemp) = (hsnet/(depth*62.4*(float(time_step)/60.)))/1.8
...
c-----the following write statements are for diagnostics purposes only
      if(print_level.ge.3)then
         if (iskip. eq. 1) then
            if (mod(julmin, 120). eq. 0) then
               ...
               if(chan_res.eq.1)then
                  if(int2ext(n). eq. 20) then
                     write(ihout,999) current_dt,int2ext(n),tsolhr,ha,
     &                    hb,he,hc,hsnet,c(mtemp),evap_rate,dryblb
                  endif
               elseif(chan_res.eq.2)then
                  write(irhout,998) current_dt,irev,tsolhr,ha,
 ...
 998  format(a,i5,f10.1,f10.2,f9.1,f10.1,f13.3,3f10.2)
      ...
      include 'param.inc'
      include 'bltm3.inc'
      real temp,tmp,frac,templastused
      integer i,it,itemp
c-----integer numcalled
