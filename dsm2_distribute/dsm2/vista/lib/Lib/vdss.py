...
def regexp_path(path):
    ...
    return path
#
def findparts(g,a='',b='',c='',d='',e='',f='',exact=1):
    """
    findparts(g,a='',b='',c='',d='',e='',f='',exact=1):
...
#
def read_dss_txt(file,dssts=1,flag=0):
    """
    read_dss_txt(file, dssts=1,flag=0): reads from a ascii file in dssts or dssits format
    and writes out the data to the appropriate dss file and pathnames.
    If dssts = 1 then dssts format is assumed in the file else dssits format is assumed
    For more info  look up doc on read_dssts(file) and read_dssits(file)
    If flag = 1 then it expects a flag value in the ascii file as well
