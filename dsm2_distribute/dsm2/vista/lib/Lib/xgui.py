...
from javax.swing.event import ChangeEvent,CellEditorListener
from javax.swing import JFrame, JTable, JPanel, JTextField, \
     ...
     CellEditor
from java.awt.event import MouseAdapter,KeyEvent,FocusListener, \
     ComponentListener
from java.lang import System
#
...
class PlotGui(JPanel,TreeSelectionListener,FocusListener):
    def __init__(self,pd=PlotData(None)):
    	...
    	#
    	dmbar = self.getMenuBar()
    	#dmbar.addFocusListener(self)
    	tpanel1 = JPanel(); tpanel1.setLayout(BorderLayout())
    	tpanel1.add(jsp); tpanel1.add(dmbar,BorderLayout.NORTH)
    	...
    	#
    	embar = self.getEMenuBar()
    	#embar.addFocusListener(self)
    	tpanel2 = JPanel(); tpanel2.setLayout(BorderLayout())
    	tpanel2.add(jsp2); tpanel2.add(embar,BorderLayout.NORTH)
    ...
    def secure_tables(self):
        if self.tbl.isEditing():
            self.tbl.getCellEditor().stopCellEditing()
        if self.tbl2.isEditing():
            self.tbl2.getCellEditor().stopCellEditing()         
    def update_to_gui(self):
            self.tbl.editingStopped(ChangeEvent(self.tbl))
            self.tbl2.editingStopped(ChangeEvent(self.tbl2))
            self.txt_loc_tbl.editingStopped(ChangeEvent(self.txt_loc_tbl))
    ...
    def focusGained(self,evt):
        ''' An entry for each table to be protected must be included here
            these tables should not have addFocusListener(self) applied
            to them.  All components (menu items for one) that conflict
            with the tables need to implement addFocusListener(self)

            This addition was made to compensate for a Java bug that causes
            a loss of table data when focus is passed to certain components
            while a table cell is still active - FIND CODE: @TBL'''
        if not evt.isTemporary():
            if self.tbl.isEditing():
                self.tbl.getCellEditor().stopCellEditing()
            if self.tbl2.isEditing():
                self.tbl2.getCellEditor().stopCellEditing()
    ...
    def getMenuBar(self):
        	...
        	em.addFocusListener(self) ## @TBL Added to protect table cell entries
        	dm.addFocusListener(self) ## @TBL Added to protect table cell entries
        	mbar = JMenuBar()
        	mbar.add(em)
        	mbar.add(dm)
        	return mbar
