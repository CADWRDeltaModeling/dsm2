...
# basic java stuff
from Acme.JPM.Encoders import GifEncoder
...
class StudyData:
    def __init__(self,name,gdata=None,init_script_data=None):
        ...
        self.root = StudyNode(self,name,1)
	self.name = name
        self.gdata=[]
        if gdata == None:
	...
	if init_script_data == None: init_script_data = ScriptData(None,'sinit')
...
class GraphData:
	...
	self.root = GraphNode(self,self.name,1)
	...
	self.root.add(PlotNode(pd,pd.name))
    def _mkroot(self):
	if self.pdata != None:
...
class ExprData(DefaultTableModel):
	...
	if exprs != None:
	    count = len(exprs)
	    for i in range(count):
		name,expr,path,legend,unit,type = names[i],exprs[i],\
						  paths[i],legends[i],\
						  units[i],types[i]
...
class PlotData(DefaultTableModel):
	...
	for x in data:
	    ...
	    if string.find(current_file,'::') < 0:
		if File(current_file).lastModified() > \
		   File(DSSUtil.getCatalogFilename(current_file)).lastModified():
	...
	for i in range(len(refs)):
	    ...
	    if isJDK2() and not attr._drawSymbol\
	       and attr._dashArray != None and len(attr._dashArray) > 1:
		crv2d = create2DCurve(refs[i],\
				      AxisAttr.BOTTOM,AxisAttr.LEFT,legtext)
		crvs.append(crv2d)
	    else:
	...
	if txts != None and len(txts) >= 0:
		...
		label,size,x,y = str(dynamic_eval(txts[i],globals(),ldict)),\
				 dynamic_eval(sizes[i],globals(),ldict),\
				 dynamic_eval(xs[i],globals(),ldict),\
				 dynamic_eval(ys[i],globals(),ldict)
