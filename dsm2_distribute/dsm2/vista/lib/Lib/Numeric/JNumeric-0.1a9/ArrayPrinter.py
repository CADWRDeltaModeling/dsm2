...
# modified by Tim Hochberg September 1998 for use with JNumeric
# (Bug workarounds and other little bits).

import sys, string
from Numeric.umath import *
import Numeric

def array2string(a, max_line_width = None, precision = None,
                                         suppress_small = None, separator=' ',
                                         array_output=0):
        if len(Numeric.shape(a)) == 0:
                #return str(a[0]) # XXX I'd argue that this is wrong.
                return str(a[()])

        if multiply.reduce(Numeric.shape(a)) == 0:
                return "zeros(%s, '%s')" % (Numeric.shape(a), a.typecode())
        ...
        if type == 'b' or type == '1' or type == 's' or type == 'i' \
           or type == 'l':
                ...
                real_format, real_item_length = _floatFormat(data.real, precision, suppress_small, sign=0)
                imag_format, imag_item_length = _floatFormat(data.imag, precision, suppress_small, sign=1)
        ...
        lst = _arrayToString(a, format_function, len(Numeric.shape(a)), line_format, 6*array_output, 0)[:-1]
        if array_output:
                #if a.typecode() in ['l', 'd', 'D']:
                if a.typecode() in [Numeric.Int, Numeric.Float, Numeric.Complex]:
                        return "array(%s)" % lst
                else:
        ...
        if rank == 0:
                ...
                for j in range(line_format[0]):
                        ...
                        for i in range(items):
                                ...
                                index = index + 1
                                if index == Numeric.shape(a)[0]: break
                        if s[-1] == ' ': s = s[:-1]
                        s = s + '\n'
                        items = line_format[2]
...
def _formatFloat(x, format, strip_zeros = 1):
    if format[-1] == '3':
        format = format[:-1]
        s = format % x
        if format[1] == '#' and '.' not in s: s = s + '.' # broken print in jpython
        third = s[-3]
        if third == '+' or third == '-':
            s = s[1:-2] + '0' + s[-2:]
    elif format[-1] == 'f':
        ...
        s = format % x
        if format[1] == '#' and '.' not in s: s = s + '.' # broken print in jpython
    s = string.replace(s, "+-", "-") # broken print in jpython
    return s
