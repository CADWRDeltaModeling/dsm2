...
// XXX Simplify signature for accumulate [ accumulate(a, b) ? ]

public class BinaryUfunc extends PyObject {
	...
	BinaryUfunc(BinaryFunction function) {
	...
	public PyObject reduce(PyObject po, int axis) {
		PyMultiarray a = PyMultiarray.swapAxes(PyMultiarray.asarray(po), axis, 0);
		...
		b = PyMultiarray.reshape(b, shape);
	...
	final public PyObject __call__(PyObject o1, PyObject o2) {
		return function.call(o1, o2);
	}
...
// XXX Change over to Multiarray._pow when that's implemented
final class Power extends BinaryFunction {
	...
	final void accumulate(Object aData, int sa, int dim, int dsa, Object rData, int sr, int rDim, int dsr, char type) {
		...
		case '1': 
			...
			byte [] rData1 = (byte[])rData, aData1 = (byte[])aData;
			for (; sa != maxSa; sa += dsa, sr += dsr)
				rData1[sr] = (last1 = (byte)Math.pow(last1,aData1[sa]));
			break;
		case 's': 
			short lasts = 1;
			short [] rDatas = (short[])rData, aDatas = (short[])aData;
			for (; sa != maxSa; sa += dsa, sr += dsr)
				rDatas[sr] = (lasts = (short)Math.pow(lasts,aDatas[sa]));
			break;
		case 'i': 
			int lasti = 1;
			int [] rDatai = (int[])rData, aDatai = (int[])aData;
			for (; sa != maxSa; sa += dsa, sr += dsr)
				rDatai[sr] = (lasti = (int)Math.pow(lasti,aDatai[sa]));
			break;
		case 'l': 
			long lastl = 1;
			long [] rDatal = (long[])rData, aDatal = (long[])aData;
			for (; sa != maxSa; sa += dsa, sr += dsr)
				rDatal[sr] = (lastl = (long)Math.pow(lastl,aDatal[sa]));
			break;
		case 'f': 
			float lastf = 1;
			float [] rDataf = (float[])rData, aDataf = (float[])aData;
			for (; sa != maxSa; sa += dsa, sr += dsr)
				rDataf[sr] = (lastf = (float)Math.pow(lastf,aDataf[sa]));
			break;
		case 'd': 
			double lastd = 1;
			double [] rDatad = (double[])rData, aDatad = (double[])aData;
			for (; sa != maxSa; sa += dsa, sr += dsr)
				rDatad[sr] = (lastd = (double)Math.pow(lastd,aDatad[sa]));
			break;
		case 'F': 
			float lastfr = 1, lastfi = 0;
			float [] rDataF = (float[])rData, aDataF = (float[])aData;
...
final class Maximum extends BinaryFunction {
	...
	final public PyObject call(PyObject po1, PyObject po2) {
		if (PyMultiarray.shapeOf(po1).length != 0 || 
			PyMultiarray.shapeOf(po2).length != 0)
			return PyMultiarray.asarray(po1).__max(PyMultiarray.asarray(po2));
		return (__builtin__.cmp(po1, po2) > 0) ? po1 : po2;
	}
	final PyObject call(PyMultiarray a, PyMultiarray b, PyMultiarray result) {
...
final class Equal extends BinaryFunction {
	...
	final public PyObject call(PyObject po1, PyObject po2) {
		...
		return (__builtin__.cmp(po1, po2) == 0) ? Py.One : Py.Zero;
	}
}
...
final class LogicalAnd extends BinaryFunction {
	...
	final public PyObject call(PyObject po1, PyObject po2) {
		...
		return (po1.__nonzero__() && po2.__nonzero__()) ? Py.One : Py.Zero;
	}
	final PyObject call(PyMultiarray a, PyMultiarray b, PyMultiarray result) {
