...
# The default version is 1.14.6, and other versions are not tested.
...
while getopts 'v:h' opt; do
    ...
    ? | h)
        ...
        echo "-v, version of HDF5 to build, e.g. 1.14.6"
        exit 1
        ;;
    esac
    shift "$(($OPTIND - 1))"
...
TOKENS=(${HDF5_VERSION//./ })
MAJOR_VERSION="${TOKENS[0]}_${TOKENS[1]}"
FULL_VERSION="${TOKENS[0]}_${TOKENS[1]}_${TOKENS[2]}"
...
URL_HDF5="https://support.hdfgroup.org/releases/hdf5/v${MAJOR_VERSION}/v${FULL_VERSION}/downloads/${HDF5_ZIP}"
echo "Downloading from $URL_HDF5"
curl -L ${URL_HDF5} -o ${HDF5_ZIP}
...
# I think there is a bug in HDF5 CMake files v1.14.6.
...
-DCMAKE_INSTALL_PREFIX=${CUR_DIR}/hdf5-${HDF5_VERSION} ..
cmake --build . --target install -j $(nproc)

# These dependency directory names are for v1.14.6.
