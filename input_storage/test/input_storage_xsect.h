...
public:
  ...
  xsect(const int & a_chan_no,const double & a_dist,const  char a_file[120], bool a_used=true, int a_layer = 0) :
    chan_no(a_chan_no),
    dist(a_dist),
    used(a_used),
    layer(a_layer)
  {
    memcpy(file,a_file,120);  
  }
...
FCALL void xsect_append_to_buffer_f(const int * a_chan_no,const double * a_dist,const  char a_file[120], int * ierror, 
              const int file_len);
