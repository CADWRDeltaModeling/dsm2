...
template<>
HDFTableManager<particle_filter>::HDFTableManager() :
    description(particle_filter_table_description()),  
    m_default_fill(particle_filter("",-901,"","","","")){}

template<>
void HDFTableManager<particle_filter>::prioritize_buffer()
{

    // Sort by identifier (lexicographical order) and
    // layer (decreasing order of priority)
    std::sort(buffer().begin(),buffer().end());
    vector<particle_filter>::const_iterator dupl = adjacent_find(buffer().begin(),buffer().end());
    if ( dupl != buffer().end())
    {   
    ...
    buffer().erase(unique(buffer().begin(),buffer().end(),identifier_equal<particle_filter>()),buffer().end());
    // Eliminate items that are not used. This must be done after lower layers have been removed
    buffer().erase(remove_if(buffer().begin(), buffer().end(),not1(entry_used<particle_filter>())), buffer().end());
    
}

TableDescription particle_filter_table_description(){
  const char* title = "particle_filter";
  const size_t size = sizeof(particle_filter);
  const size_t nfields = 6;
  particle_filter default_struct = particle_filter("",-901,"","","","");
  const char* fnames[] =  {"name","node","at_wb","fillin","file","path"};
  const hid_t ftypes[] =  {
            string_type(32),H5T_NATIVE_INT,string_type(32),string_type(8),string_type(128),string_type(80)
               };
...
/**
  Clear the storage buffer for objects of type particle_filter
*/  
void particle_filter_clear_buffer_f(){
  //particle_filter_table::instance().buffer().destroy();
  particle_filter_table::instance().buffer().clear();
}

/** append to buffer, compatible with fortran, returns new size*/
void particle_filter_append_to_buffer_f(const  char a_name[32],const int * a_node,const  char a_at_wb[32],const  char a_fillin[8],const  char a_file[128],const  char a_path[80], int * ierror, 
              const int name_len,const int at_wb_len,const int fillin_len,const int file_len,const int path_len)
{
 _TRAP_EXCEPT(*ierror,
   particle_filter_table::instance().buffer().push_back(
                                      particle_filter(
                                      a_name,*a_node,a_at_wb,a_fillin,a_file,a_path
                                      ));
 ) // end of exception trap
}
  
/** both makes the table and writes the contents of the buffer to it */
void particle_filter_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  particle_filter_table & table = particle_filter_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( particle_filter_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //particle_filter_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void particle_filter_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    particle_filter_table & table = particle_filter_table::instance();
...
void particle_filter_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     particle_filter_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
void particle_filter_query_from_buffer_f(size_t* row, 
...
{
 _TRAP_EXCEPT(*ierror,
    //if (row > particle_filter_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    particle_filter obj =particle_filter_table::instance().buffer()[ndx];
...
void particle_filter_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  particle_filter_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type particle_filter */
int particle_filter_buffer_size_f()
{ 
  return (int) particle_filter_table::instance().buffer().size();
}

void particle_filter_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("particle_filter");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<particle_filter> & obs = particle_filter_table::instance().buffer();
   particle_filter_table& table = particle_filter_table::instance();
   ...
   out << endl;
   for (vector<particle_filter>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const particle_filter & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void particle_filter_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  particle_filter_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
