#ifndef rate_coefficient_STORAGE_H__
#define rate_coefficient_STORAGE_H__
/**
...
/** Structure representing input of type rate_coefficient.
...
public:
  ...
  /** Data type rate_coefficient, default constructor */  
  ...
  /** Construct a rate_coefficient with actual data values */
  rate_coefficient(const  char a_group_name[32],const  char a_constituent[16],const  char a_variable[16],const double & a_value, bool a_used=true, int a_layer = 0) :
    value(a_value),
    used(a_used),
    layer(a_layer)
  {
  ...
  /**Copy constructor) 
   */
  rate_coefficient (const rate_coefficient & other) :
    value(other.value),
    used(other.used),
    layer(other.layer)
  {
  ...
  /** Parent object class name.
      ...
      of this class.
  */
  rate_coefficient::identifier_type parent_identifier()  const
  {
     return rate_coefficient::identifier_type( group_name,constituent,variable );
  }

  /** Return the version/layer number of the parent object */ 
  int parent_version()  const
  {
    vector<rate_coefficient>& pbuf = HDFTableManager<rate_coefficient>::instance().buffer();
    rate_coefficient parent;
    parent.set_identifier(parent_identifier());
    vector<rate_coefficient>::iterator loc = lower_bound(pbuf.begin(),
                                                pbuf.end(),
                                                parent,
                                                identifier_compare<rate_coefficient>());
  ...
  bool operator< (const rate_coefficient & other) const
  {
  ...
  rate_coefficient& operator=(const rate_coefficient& rhs)
  {
  ...
  /** Return the class name of this object (rate_coefficient) */
...
typedef HDFTableManager<rate_coefficient> rate_coefficient_table;
...
*/  
FCALL void rate_coefficient_clear_buffer_f();

/** query number of records being stored in buffer */
FCALL int rate_coefficient_buffer_size_f();


/** append to buffer, compatible with fortran, returns new size*/
FCALL void rate_coefficient_append_to_buffer_f(const  char a_group_name[32],const  char a_constituent[16],const  char a_variable[16],const double * a_value, int * ierror, 
              const int group_name_len,const int constituent_len,const int variable_len);
  
/** both makes the table and writes the contents of the buffer to it */
FCALL void rate_coefficient_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror);

/** reads the table in from a file into the buffer*/
FCALL void rate_coefficient_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror);

/** query size information about the table in hdf5
*/
FCALL void rate_coefficient_number_rows_hdf5_f(const hid_t* file_id, hsize_t* nrecords, int* ierror);


/** get one row worth of information from the buffer */
FCALL void rate_coefficient_query_from_buffer_f(size_t* row, 
...
/**
  ...
  */
FCALL void rate_coefficient_prioritize_buffer_f(int* ierror);
/**
 ...
 */
FCALL void rate_coefficient_write_buffer_to_text_f(const char* file, const bool* append, int* ierror, int filelen);
