...
void HDFTableManager<suspended_sediment>::prioritize_buffer()
{
    ...
    vector<suspended_sediment>::const_iterator dupl = adjacent_find(buffer().begin(),buffer().end());
    if ( dupl != buffer().end())
    {   
    ...
    buffer().erase(unique(buffer().begin(),buffer().end(),identifier_equal<suspended_sediment>()),buffer().end());
    // Eliminate items that are not used. This must be done after lower layers have been removed
    buffer().erase(remove_if(buffer().begin(), buffer().end(),not1(entry_used<suspended_sediment>())), buffer().end());
...
/**
  Clear the storage buffer for objects of type suspended_sediment
*/  
void suspended_sediment_clear_buffer_f(){
  //suspended_sediment_table::instance().buffer().destroy();
  suspended_sediment_table::instance().buffer().clear();
}
...
void suspended_sediment_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  suspended_sediment_table & table = suspended_sediment_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( suspended_sediment_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //suspended_sediment_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void suspended_sediment_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    suspended_sediment_table & table = suspended_sediment_table::instance();
...
void suspended_sediment_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     suspended_sediment_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
{
 _TRAP_EXCEPT(*ierror,
    //if (row > suspended_sediment_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    suspended_sediment obj =suspended_sediment_table::instance().buffer()[ndx];
...
void suspended_sediment_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  suspended_sediment_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type suspended_sediment */
int suspended_sediment_buffer_size_f()
{ 
  return (int) suspended_sediment_table::instance().buffer().size();
}

void suspended_sediment_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("suspended_sediment");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<suspended_sediment> & obs = suspended_sediment_table::instance().buffer();
   suspended_sediment_table& table = suspended_sediment_table::instance();
   ...
   out << endl;
   for (vector<suspended_sediment>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const suspended_sediment & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void suspended_sediment_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  suspended_sediment_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
