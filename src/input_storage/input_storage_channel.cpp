...
istream& operator>> (istream& stream, channel & obj)
{
  ...
  string tempstr;
        ...
        tempstr = *(beg++);
        tokenstrm.str(tempstr);
        tokenstrm >> obj.length;  // strtol(tempStr.c_str(),NULL,10);
        if (!tokenstrm.eof())
        {
          throw invalid_argument("Could not convert length to correct data type:"+tempstr);
        }
        ...
        obj.dispersion = strtod((beg++)->c_str(),NULL);
...
HDFTableManager<channel>::HDFTableManager() :
    ...
    m_default_fill(channel(-901,-901,-901.0,-901.0,-901,-901)){}
...
TableDescription channel_table_description(){
  ...
  channel default_struct = channel(-901,-901,-901.0,-901.0,-901,-901);
  const char* fnames[] =  {"chan_no","length","manning","dispersion","upnode","downnode"};
  const hid_t ftypes[] =  {
            H5T_NATIVE_INT,H5T_NATIVE_INT,H5T_NATIVE_DOUBLE,H5T_NATIVE_DOUBLE,H5T_NATIVE_INT,H5T_NATIVE_INT
               };

  const size_t foffsets[] ={
             ((char*)&default_struct.chan_no - (char*)&default_struct),
             ((char*)&default_struct.length - (char*)&default_struct),
             ((char*)&default_struct.manning - (char*)&default_struct),
             ((char*)&default_struct.dispersion - (char*)&default_struct),
  ...
  const size_t fsizes[] = {
         ...
         sizeof( default_struct.length ),
         sizeof( default_struct.manning ),
         sizeof( default_struct.dispersion ),
...
void channel_append_to_buffer_f(const int * a_chan_no,const int * a_length,const double * a_manning,const double * a_dispersion,const int * a_upnode,const int * a_downnode, int * ierror)
{
 _TRAP_EXCEPT(*ierror,
   channel_table::instance().buffer().push_back(
                                      channel(
                                      *a_chan_no,*a_length,*a_manning,*a_dispersion,*a_upnode,*a_downnode
                                      ));
