#ifndef envvar_STORAGE_H__
#define envvar_STORAGE_H__
/**
...
/** Structure representing input of type envvar.
...
public:
  ...
  /** Data type envvar, default constructor */  
  ...
  /** Construct a envvar with actual data values */
  envvar(const  char a_name[32],const  char a_value[128], bool a_used=true, int a_layer = 0) :
    
    used(a_used),
    layer(a_layer)
  {
    memcpy(name,a_name,32);
    memcpy(value,a_value,128);  
  }
  
  /**Copy constructor) 
   */
  envvar (const envvar & other) :
    
    used(other.used),
    layer(other.layer)
  {
    memcpy(name,other.name,32);
    memcpy(value,other.value,128);  
  }
  ...
  /** Parent object class name.
      ...
      of this class.
  */
  envvar::identifier_type parent_identifier()  const
  {
     return envvar::identifier_type( name );
  }

  /** Return the version/layer number of the parent object */ 
  int parent_version()  const
  {
    vector<envvar>& pbuf = HDFTableManager<envvar>::instance().buffer();
    envvar parent;
    parent.set_identifier(parent_identifier());
    vector<envvar>::iterator loc = lower_bound(pbuf.begin(),
                                                pbuf.end(),
                                                parent,
                                                identifier_compare<envvar>());
  ...
  envvar& operator=(const envvar& rhs)
  {
  ...
  /** Return the class name of this object (envvar) */
...
typedef HDFTableManager<envvar> envvar_table;
...
*/  
FCALL void envvar_clear_buffer_f();

/** query number of records being stored in buffer */
FCALL int envvar_buffer_size_f();


/** append to buffer, compatible with fortran, returns new size*/
FCALL void envvar_append_to_buffer_f(const  char a_name[32],const  char a_value[128], int * ierror, 
              const int name_len,const int value_len);
  
/** both makes the table and writes the contents of the buffer to it */
FCALL void envvar_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror);

/** reads the table in from a file into the buffer*/
FCALL void envvar_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror);

/** query size information about the table in hdf5
*/
FCALL void envvar_number_rows_hdf5_f(const hid_t* file_id, hsize_t* nrecords, int* ierror);


/** get one row worth of information from the buffer */
FCALL void envvar_query_from_buffer_f(size_t* row, 
                         char a_name[32], char a_value[128], int * ierror, 
              int name_len,int value_len);
/**
  prioritize buffer by layers, delete unused items and sort
  */
FCALL void envvar_prioritize_buffer_f(int* ierror);
/**
 ...
 */
FCALL void envvar_write_buffer_to_text_f(const char* file, const bool* append, int* ierror, int filelen);
