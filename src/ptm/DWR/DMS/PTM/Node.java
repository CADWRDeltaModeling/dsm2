 ...
 * Node
 * 
 * Node is defined as the connection between two or more waterbodies. Node
 * handles connection information as well as constructing outflow from or to
 * kind of information
 * <p>
 * 
 * FUTURE DIRECTIONS
...
public class Node {
	 ...
	 * NOT USED; replaced by getTotalEffectiveOutflow
	 */
	...
	/**
	 * Get total positive outflow from Node.. add up all flows leaving the Node
	 * multiplied by a filter operation for each out-node filter
	 * for particle decision making when facing multiple waterbodies outflow from one node
	 */
	public final float getTotalEffectiveOutflow(boolean addSeep) {
		float outflow = 0.0f;
		// System.out.println(this.toString());
		// for each Waterbody connected to junction add the outflows
		for (int id = 0; id < numberOfWaterbodies; id++) {
			if (!addSeep) { // @todo: hobbled seep feature
				// if(getWaterbody(id).getAccountingType() != flowTypes.evap){
				outflow += getFilterOp(id) * getOutflow(id);
				// }
			} else {
				outflow += getFilterOp(id) * getOutflow(id);
			}
	...
	/**
	 * get the operation for the specified filter at current timestamp
	 * used to modify the outflows' weights for particle decision making
	 */
	public float getFilterOp(int localChannelId) {
        float filterOp = 1.0f;
        int wbEnvIndex = this.getWaterbodyEnvIndex(localChannelId);
        String key = ""+getEnvIndex()+","
                    +""+wbEnvIndex;
	...
	/**
	 * existence of particle filter for node inflows
	 * called by Particle x-position calculation when entering a node
	 */
	...
	/**
	 * Gets the outflow (positive) to a particular Waterbody from this Node. It
	 * returns a zero for negative outflow or inflow
	 */
	...
	/**
	 * Adds Waterbody of given PTMEnv index to Node wbIndexArray. These
	 * operations should be done prior to calling setWbArray
	 */
	...
	/**
	 * Boundary array as defined from fixed input. This is not needed for
	 * boundary Waterbody information as only waterbodies can be boundaries.
	 */
