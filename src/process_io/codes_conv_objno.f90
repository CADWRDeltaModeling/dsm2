...
module codes_conv_objno

    contains
    
    !> get internal object number, given object type and external (map) ID
    integer function name_to_objno(obj_type, name)
    
        use common_variables
        use common_dsm2_vars
        
        integer, intent(in) :: obj_type       !< object type
        character*32, intent(in) :: name      !< name/identifier
        ...
        if (obj_type .eq. obj_reservoir) then
            do i = 1,n_resv
                if (resv_geom(i)%name .eq. tname) then
                    name_to_objno = i
                    return
                end if
            end do
        else if (obj_type .eq. obj_gate) then
         do i = 1,n_gate
            if (gate(i)%name .eq. tname) then
               name_to_objno = i
               return
            end if
         end do
      else if (obj_type .eq. obj_channel) then
         read(tname,'(i)') i
         name_to_objno = i
      else if (obj_type .eq. obj_node) then
         read(tname,'(i)') i
         name_to_objno = i
      else if (obj_type .eq. obj_obj2obj) then
         do i= 1,n_transfer
            if (tran(i)%name .eq. tname) then
               name_to_objno = i
               return
            end if
         end do
      else if (obj_type .eq. obj_qext) then
         do i=1, n_qext
            if (qext(i)%name .eq. tname) then
               name_to_objno = i
               return
            end if
         end do
      !else if (obj_type .eq. obj_stage) then
      !   do i=1,n_stgbnd
      !      if (stgbnd(i)%name .eq. tname) then
      !         name_to_objno = i
      !         return
      !      end if
       !  end do
      else if (obj_type .eq. obj_group) then
         !do i=0,ngroup
