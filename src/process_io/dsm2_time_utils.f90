...
module dsm2_time_utils
    ...
    subroutine incr_intvl(incr_intv, jmins, e_part, boundary)
      ...
      integer, intent(out) :: incr_intv         !> incremented interval
      character, intent(in) :: e_part*(*)       !> DSS style interval
      integer, intent(in) :: jmins              !> starting julian minute
     ...
     integer*4 :: iymdjl, &              ! DSS function
      ...
      integer :: number,            &  ! number (e.g. 1, 15)
...
!--------------now back to same or previous 15min boundary
 ...
 100     continue
    ...
    subroutine split_epart(e_part,number,interval)
        ...
        integer, intent(out) :: number         ! number of intervals        
 ...
 600    continue                  ! here for error getting number of intervals
    ...
    subroutine incr_intvl_r(incr_intv, jmins, e_part, boundary)
      ...
      character :: e_part*(*),    & ! DSS style interval [INPUT]
                   ...
                   interval*80      ! DSS interval (e.g. HOUR)
      real(gtm_real), intent(out) :: incr_intv             
      ...
      do i=1,nfields
         ...
         call split_epart_r(e_part_tmp(ibegf(i):ibegf(i)+ilenf(i)),number,   &
              interval)
 ...
 100     continue
    ...
    !> six 1HOUR intervals, not a single interval of 6HOUR).
    !> However 15MIN means a single 15MIN interval.      
    subroutine split_epart_r(e_part,number,interval)
        use common_dsm2_vars, only: miss_val_i
        implicit none
