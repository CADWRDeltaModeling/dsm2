...
module sediment_bed_setup
    ...
    use common_variables , only: n_cell, n_resv, n_chan, group, n_group, obj_channel, obj_reservoir, chan_geom, hydro_hdf5
    use common_dsm2_vars, only: gtm, io_write, io_restart, io_hdf5, io_read, io_files
    use sed_bed_hdf
    use common_dsm2_vars, only: dsm2_name, dsm2_modifier  
    implicit none
    ...
    integer, parameter :: sb_inter_frac_tb = 4
    integer, parameter :: sb_q10_ct_inter = 5
    integer, parameter :: sb_tb_ct_inter = 6
    integer, parameter :: sb_porosity = 7
                ...
                if (group(group_id)%member_pattern_code(ii) .eq. obj_channel) then
                    ...
                    if (chan_number<=n_chan) then
                        do jj= chan_geom(chan_number)%start_cell,chan_geom(chan_number)%end_cell
                           ! do kk = 1, n_zones
                                call assign_sed_input_variable(jj, sed_layer, var_id, value)
    ...
    subroutine sed_input_variable(variable, sb_variable_id)
     ...
     subroutine assign_sed_input_variable(cell_no, sed_layer, sb_variable_id, value)
        ...
        case (sb_inter_frac_tb)
            bed(cell_no,:, sed_layer).inter_frac_tb = value
