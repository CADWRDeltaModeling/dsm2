...
!==============================================================================
    ...
    subroutine advect_single_channel(mass,     &
                      mass_prev,&
                      flow_prev,&      
                      flow_lo,  &
                      flow_hi,  &
    ...
    ! Calculate the (undivided) differences of concentrations
    call difference_single_channel(grad_lo,    &
                    grad_hi,    &
                    grad_center,&
                    conc_prev,  &
                    ncell,      &
    ...
    ! Applies flux-limeter on high resolution gradient 
        call limiter_single_channel(grad_lim,   &
                     grad_lo,    &
                     grad_hi,    &
                     grad_center,&
                     ncell,      &
    ...
    call compute_source(source_prev, & 
                        ...
                        old_time)
    ! Extrapolate primitive data from cell center at the old time
    call extrapolate_single_channel(conc_lo,     &
                     conc_hi,     & 
                     conc_prev,   &
                     grad,        &            
                     source_prev, &
    ...
    !> a Taylor series in time and space in which an explicit discretization
    !> of the PDE is used to represent the time part.
    pure subroutine extrapolate_single_channel(conc_lo,  &
                                conc_hi,  &
                                conc,     &
                                grad,     &
                                source,   &                       
