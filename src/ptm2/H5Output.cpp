...
void H5Output::writeGrid(const Grid& grid)
{
  ...
  hid_t geometryGroupId = H5Gcreate(fileId_, groupName.c_str(),  lcpl_id, gcpl_id, gapl_id);
  ...
  BOOST_FOREACH(const Channel* channel, grid.getChannels()) {
    channels[i] = boost::lexical_cast<int>(channel->getExternalId());
    ++i;
  }
  int rank = 1;
  hsize_t dims[1];
  ...
  writeData(dataName, geometryGroupId, rank, dims, channels);
  delete [] channels;
...
template<typename T>
void H5Output::writeData(std::string& name, hid_t hid, const int rank, const hsize_t* dims, const T* data)
{
  hid_t spaceId = H5Screate_simple(rank, dims, dims);
  hid_t dsetId = H5Dcreate(hid,  name.c_str(), H5T_NATIVE_INT, spaceId, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  hsize_t offset[1];
  offset[0] = 0;
  hsize_t stride[1];
  stride[0] = 0;
...
void H5Output::createTimeStep(const int& julmin)
{
  ...
  if (!trial) {
    base_gid = H5Gcreate(fileId_, baseGroupName.c_str(), lcpl_id, gcpl_id, gapl_id);
  }
  else {
  ...
  currentTimeGroupId_ = H5Gcreate(base_gid, groupName.c_str(), lcpl_id, gcpl_id, gapl_id);
  if (currentTimeGroupId_ < 0) {
    throw std::runtime_error("H5Output::createTimeStep: cannot create a group for a time step.");
  }
  ++nSteps_;
  ...
  hid_t attrId = H5Acreate(currentTimeGroupId_, "jul_min", H5T_NATIVE_INT, spaceId, H5P_DEFAULT,
    H5P_DEFAULT);
...
void H5Output::writeParticles(const std::vector<Particle*>& particles)
{
  ...
  hid_t dsetId = H5Dcreate(currentTimeGroupId_, "waterbody_type", H5T_NATIVE_INT, spaceId, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  hsize_t offset[1];
  offset[0] = 0;
  hsize_t stride[1];
  stride[0] = 0;
  ...
  H5Sselect_hyperslab(spaceId, H5S_SELECT_SET, offset, NULL, counts, NULL);
  H5Dwrite(dsetId, H5T_NATIVE_INT, spaceId, H5S_ALL, H5P_DEFAULT, &intCache_[0]);
...

  {
    ...
    hid_t attrId = H5Acreate(currentTimeGroupId_, "n_particles", H5T_NATIVE_INT, nullSpaceId, H5P_DEFAULT,
      H5P_DEFAULT);
  ...
  // body id
  {
    if (nLiveParticles != 0) {
      ...
      dims[0] = nLiveParticles;
      hid_t spaceId = H5Screate_simple(rank, dims, dims);
      hid_t dsetId = H5Dcreate(currentTimeGroupId_, "waterbody_id", H5T_NATIVE_INT, spaceId, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
      hsize_t offset[1];
      offset[0] = 0;
      hsize_t stride[1];
      stride[0] = 0;
  ...
  dims[0] = nLiveParticles;
  hid_t spaceId = H5Screate_simple(rank, dims, dims);
  hid_t dsetId = H5Dcreate(currentTimeGroupId_, "x", H5T_NATIVE_DOUBLE, spaceId, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  hsize_t offset[1];
  offset[0] = 0;
  hsize_t stride[1];
  stride[0] = 0;
  ...
  counts[0] = nLiveParticles;
  H5Sselect_hyperslab(spaceId, H5S_SELECT_SET, offset, NULL, counts, NULL);
  H5Dwrite(dsetId, H5T_NATIVE_DOUBLE, spaceId, H5S_ALL, H5P_DEFAULT, &doubleCache_[0]);
