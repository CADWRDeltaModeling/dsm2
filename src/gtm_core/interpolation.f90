...
module interpolation
   ...
   !> Interpolated flow and area mesh based on given four points of flows and water surface elevations
    ...
    !> This will return the interpolated values for all cells
    subroutine interp_cell(prev_flow_arr, prev_ws_arr, flow_arr, ws_arr, hydro_dt, dx, dt)
        use common_variables, only : n_segm, segm
        use common_xsect
        implicit none
        real(gtm_real), intent(in) :: prev_flow_arr
        real(gtm_real), intent(in) :: prev_ws_arr
        real(gtm_real), intent(in) :: flow_arr 
        real(gtm_real), intent(in) :: ws_arr
        real(gtm_real), intent(in) :: hydro_dt
        real(gtm_real) :: dx
        real(gtm_real) :: dt 
        integer :: nt
