...
module boundary_diffusion_network
    ...
    subroutine network_boundary_diffusive_flux_prev(diffusive_flux_lo, &
        ...
        use state_variables_network, only : current_prev_node_conc, current_prev_conc_stip
        implicit none
        !--- args
        integer,intent(in)  :: ncell                                 !< Number of cells
        integer,intent(in)  :: nvar                                  !< Number of variables        
  ...
  !> Adjustments for network diffusion matrix
    subroutine network_diffusion_sparse_matrix(center_diag ,       &
        ...
        use state_variables_network, only : node_conc, current_prev_node_conc
        ...
        do i = 1, ncell
            if (typ(j).eq."u") then              ! "u": upstream boundary 
                ...
                if (current_prev_node_conc(ncc(j),1).eq.LARGEREAL)  current_prev_node_conc(ncc(j),1) = zero
                ...
                exp_diffusion_op_minus(:) = -(one-theta_gtm)*dt*area_lo_prev(i)*disp_coef_lo_prev(i) &
                                            *(conc_prev(i,:)-current_prev_node_conc(ncc(j),:))/(half*dx(i))/dx(i)  
            ...
            elseif (typ(j).eq."d") then         ! "d": downstream boundary
                ...
                if (current_prev_node_conc(ncc(j),1).eq.LARGEREAL) current_prev_node_conc(ncc(j),1) = zero           
                exp_diffusion_op_plus(:) = -(one-theta_gtm)*dt*area_hi_prev(i)*disp_coef_hi_prev(i) &
                                            *(current_prev_node_conc(ncc(j),:)-conc_prev(i,:))/(half*dx(i))/dx(i)
                exp_diffusion_op_minus(:) = -(one-theta_gtm)*dt*area_lo_prev(i)*disp_coef_lo_prev(i) &
            ...
            elseif (typ(j).eq."h" .and. flow_hi(i).ge.zero) then        ! "h": u/s of junction  
                ...
                do k = j, j + nco(j) - 1
                    ...
                    elseif ((uds(k).eq."d" .and. flow_lo(col(k)).gt.zero).or.  & 
                            (uds(k).eq."f" .and. flow_hi(col(k)).lt.zero)) then
                        aax(h) = -theta_gtm*dt*min(area_hi(i),area_lo(col(k)))*disp_coef_hi(i) &
                                /dx(i)/(half*dx(i)+half*dx(col(k)))           
                    elseif (uds(k).eq."n") then
                        aax(h) = area(i) + theta_gtm*dt*area_lo(i)*disp_coef_lo(i)/dx(i)/(half*dx(i)+half*dx(i-1))
                        do kk = j, j + nco(j) - 1
                            if ((uds(kk).eq."d" .and. flow_lo(col(kk)).gt.zero).or.   &
                                (uds(kk).eq."f" .and. flow_hi(col(kk)).lt.zero)) then         
                                 aax(h) = aax(h) + theta_gtm*dt*min(area_hi(i),area_lo(col(kk)))*disp_coef_hi(i) &
                                                /dx(i)/(half*dx(i)+half*dx(col(kk)))                                               
            ...
            elseif (typ(j).eq."h" .and. flow_hi(i).lt.zero) then        ! "h": u/s of junction  
                ...
                do k = j, j + nco(j) - 1
                    ...
                    if (uds(k).eq."u") then
                        aax(h) = -theta_gtm*dt*area_lo(i)*disp_coef_lo(i)/dx(i)/(half*dx(i)+half*dx(i-1))                    
                    elseif ((uds(k).eq."d" .and. flow_lo(col(k)).lt.zero).or.     &
                            (uds(k).eq."f" .and. flow_hi(col(k)).gt.zero)) then
                        aax(h) = -theta_gtm*dt*min(area_hi(i),area_lo(col(k)))*disp_coef_hi(i) &
                                /dx(i)/(half*dx(i)+half*dx(col(k)))              
                    elseif (uds(k).eq."n") then
                        aax(h) = area(i) + theta_gtm*dt*area_lo(i)*disp_coef_lo(i)/dx(i)/(half*dx(i)+half*dx(i-1))
                        do kk = j, j + nco(j) - 1
                            if ((uds(kk).eq."d" .and. flow_lo(col(kk)).lt.zero).or.  &
                                (uds(kk).eq."f" .and. flow_hi(col(kk)).gt.zero)) then
                                aax(h) = aax(h) + theta_gtm*dt*min(area_hi(i),area_lo(col(kk)))*disp_coef_hi(i)  &
                                                /dx(i)/(half*dx(i)+half*dx(col(kk)))                                             
                                exp_diffusion_op_plus(:) = exp_diffusion_op_plus(:)                               &
                                                          -(one-theta_gtm)*dt*area_hi_prev(i)*disp_coef_hi_prev(i) &
            ...
            elseif (typ(j).eq."l" .and. flow_lo(i).ge.zero) then       ! "l": d/s of junction
                ...
                do k = j, j + nco(j) - 1
                    ...
                    elseif ((uds(k).eq."u" .and. flow_hi(col(k)).gt.zero).or.  &
                            (uds(k).eq."f" .and. flow_lo(col(k)).lt.zero)) then 
                    ...
                    elseif (uds(k).eq."n") then
                        ...
                        do kk = j, j + nco(j) - 1
                            if ((uds(kk).eq."u" .and. flow_hi(col(kk)).gt.zero).or. &
                                (uds(kk).eq."f" .and. flow_lo(col(kk)).lt.zero)) then  
            ...
            elseif (typ(j).eq."l" .and. flow_lo(i).lt.zero) then       ! "l": d/s of junction
                ...
                do k = j, j + nco(j) - 1
                    ...
                    if (uds(k).eq."d") then
                        aax(h) = -theta_gtm*dt*area_hi(i)*disp_coef_hi(i)/dx(i)/(half*dx(i)+half*dx(i+1))               
                    elseif ((uds(k).eq."u" .and. flow_hi(col(k)).lt.zero).or.  &            
                            (uds(k).eq."f" .and. flow_lo(col(k)).gt.zero)) then
                        aax(h) = -theta_gtm*dt*min(area_lo(i),area_hi(col(k)))*disp_coef_lo(i)  &
                                /dx(i)/(half*dx(i)+half*dx(col(k)))                           
                    elseif (uds(k).eq."n") then
                        aax(h) = area(i) + theta_gtm*dt*area_hi(i)*disp_coef_hi(i)/dx(i)/(half*dx(i)+half*dx(i+1))
                        do kk = j, j + nco(j) - 1
                            if ((uds(kk).eq."u" .and. flow_hi(col(kk)).lt.zero).or.&
                                (uds(kk).eq."f" .and. flow_lo(col(kk)).gt.zero)) then                                                         
                                aax(h) = aax(h) + theta_gtm*dt*min(area_lo(i),area_hi(col(kk)))*disp_coef_lo(i)           &
                                                /dx(i)/(half*dx(i)+half*dx(col(kk))) 
                                exp_diffusion_op_plus(:) = exp_diffusion_op_plus(:)                                &
                                                         -(one-theta_gtm)*dt*area_lo_prev(i)*disp_coef_lo_prev(i)  &
    ...
    subroutine network_diffusion_sparse_matrix_zero_at_junctions(center_diag ,       &
        ...
        use error_handling
        use common_variables, only : n_node, dsm2_network, constituents
        use state_variables_network, only : node_conc, current_prev_node_conc, conc_stip, current_prev_conc_stip
        use utils, only: sparse2matrix
        implicit none
        !--- args                        
        integer, intent(in) :: ncell                                   !< Number of cells
        ...
        do i = 1, ncell
            if (typ(j).eq."u") then              ! "u": upstream boundary 
                ...
                do ivar = 1, nvar
                if (constituents(ivar)%simulate) then
                if (current_prev_node_conc(ncc(j),ivar).eq.LARGEREAL)  current_prev_node_conc(ncc(j),ivar) = zero
                extrp_prev(ivar) = conc_prev(i,ivar)-half*(conc_prev(i+1,ivar)-conc_prev(i,ivar))                
                exp_diffusion_op_plus(ivar)  = -(one-theta_gtm)*dt*area_hi_prev(i)*disp_coef_hi_prev(i) &
                                            *(conc_prev(i+1,ivar)-conc_prev(i,ivar))/x_int/dx(i)
                exp_diffusion_op_minus(ivar) = -(one-theta_gtm)*dt*area_lo_prev(i)*disp_coef_lo_prev(i) &
    ...
    !> Adjustments for network diffusion matrix by assuming zero diffusive flux across junctions
    subroutine network_diffusion_sparse_matrix_dirichlet_at_junctions(center_diag ,       &
                                                                 up_diag,            &     
                                                                 down_diag,          &
                                                                 right_hand_side,    &
                                                                 explicit_diffuse_op,&
        ...
        use error_handling
        use common_variables, only : n_node, dsm2_network
        use state_variables_network, only : node_conc, conc_stip, current_prev_conc_stip, current_prev_node_conc
        use utils, only: sparse2matrix
        implicit none
        !--- args                        
        integer, intent(in) :: ncell                                   !< Number of cells
        ...
        do i = 1, ncell
            ...
            elseif (typ(j).eq."h") then        ! "h": u/s of junction  
                ...
                do k = j, j + nco(j) - 1
                    ...
                    elseif (uds(k).eq."n") then
                        aax(h) = area(i) + theta_gtm*dt*area_hi(i)*disp_coef_hi(i)/dx(i)/(dx(i)*half)+             &
                                           theta_gtm*dt*area_lo(i)*disp_coef_lo(i)/dx(i)/(half*dx(i)+half*dx(i-1))                  
                    end if                                
                end do
                if (current_prev_conc_stip(i,1).ne.LARGEREAL .and. conc_stip(i,1).ne.LARGEREAL) then
                    extrp_prev(:) = current_prev_conc_stip(i,:)
                else
                    extrp_prev(:) = conc_prev(i,:)+half*(conc_prev(i,:)-conc_prev(i-1,:))
                end if    
                exp_diffusion_op_plus(:) = -(one-theta_gtm)*dt*area_hi_prev(i)*disp_coef_hi_prev(i) &
            ...
            elseif (typ(j).eq."l") then        ! "l": d/s of junction
                ...
                do k = j, j + nco(j) - 1
                    ...
                    elseif (uds(k).eq."n") then
                        aax(h) = area(i) + theta_gtm*dt*area_lo(i)*disp_coef_lo(i)/dx(i)/(dx(i)*half)+             &
                                           theta_gtm*dt*area_hi(i)*disp_coef_hi(i)/dx(i)/(half*dx(i)+half*dx(i+1))
                    end if                    
                end do 
                if (current_prev_conc_stip(i,1).ne.LARGEREAL .and. conc_stip(i,1).ne.LARGEREAL) then !???
                    extrp_prev(:) = current_prev_conc_stip(i,:)
  ...
  !> Adjustments for network diffusion matrix
    subroutine network_diffusion_sparse_geom(ncell)
        use gtm_precision
        use error_handling
        use common_variables, only : cell, n_node, n_junc, dsm2_network
        ...
        do k = 1, n_nonzero
            ...
            if (typ(k).eq.'s') then
                write(*,*) "Problem with stagnation node. Please check your network settings. Channel No.",cell(row(k))%chan_no
                call exit(-3)
            end if
        end do
