...
1.  **Create the reservoir:**
    1.  In Windows Explorer, navigate to the
        directory: *\\{DSM2_home}\tutorial\simple\t2_reservoir_gate_transfer*.<BR>
...
1.  1.  Enter the following values into the appropriate fields:<BR>
        1.  Name: *res_1*<BR>
        2.  Area (million sq ft): *40*<BR>
...
1.  1.  Enter the following values into the appropriate fields for the
        ...
        1.  Res Name: res_1<BR>
        2.  Node: *3*<BR>
        3.  Res Coef (in): *200*<BR>
        4.  Res Coef (out): *200*<BR>
...
1.  **Add Initial Conditions for the Reservoir:**
    1.  Create the *Reservoir Initial Conditions* *table*:<BR>
...
1.  **Create the Gate:**
    1.  Now we are going to create the GATE table and its child table
        ...
        the *GATE_DEVICE* table.<BR>
    2.  At the bottom of hydro.inp, add the skeleton for the GATE table:<BR>

GATE  
NAME FROM_OBJ FROM_IDENTIFIER TO_NODE  
END

1.  1.  In the *Gates* *table*:<BR>
        1.  Add a row and enter the following values into the
            appropriate fields:<BR>
            1.  Name: *gate_1*<BR>
            2.  From object: *channel*<BR>
            3.  From identifier: *2 \[note that this 2 refers to channel
                2\]*<BR>
            4.  to Node: *2 \[note that this 2 refers to node 2\]*<BR>
        2.  Create a GATE_WEIR_DEVICE table with the following fields:<BR>

GATE_NAME, DEVICE, NDUPLICATE, WIDTH, ELEV, HEIGHT, CF_FROM_NODE,
CF_TO_NODE, DEFAULT_OP<BR>

1.  Enter the following values into the appropriate fields:<BR>
    1.  Gate Name: gate_1<BR>
    2.  Device: *weir*<BR>
    3.  NDuplicate: *2*<BR>
    4.  Width: *20*<BR>
    5.  Elev: *2*<BR>
...
1.  Create a GATE_PIPE_DEVICE table by looking up the
    ...
    2.  Again, in the *Gate Devices* table:<BR>
        1.  On a new line enter the following values into the
            appropriate fields:<BR>
            1.  Gate Name: gate_1<BR>
            2.  Device Name: *pipe*<BR>
            3.  Number of duplicates: *2*<BR>
            4.  Radius: *2*<BR>
            5.  Elevation: *2*<BR>
            6.  Flow coefficient from Node: *0.8*<BR>
...
1.  Enter the following values into the appropriate fields:<BR>
        1.  Name: *transfer_1*<BR>
        2.  From Object: *reservoir*<BR>
        3.  To identifier: *res_1*<BR>
        4.  To Object: *node*<BR>
        5.  To identifier: *6*<BR>
...
1.  In hydro.inp, create the *Transfer Time Series* table:<BR>
    1.  The headers are:
...
1.  Enter the following values into the appropriate fields:<BR>
        1.  Input Name: *transfer_1*<BR>
        2.  Fillin: *last*<BR>
        3.  Input File: *constant*<BR>
        4.  Path/Value: *40*<BR>
...
1.  **Running HYDRO and QUAL**
    1.  In Windows Explorer, navigate to the directory: \_  <BR>
          
        Unknown macro: {DSM2_home}tutorialsimple{\_}.<BR>
    2.  Right-click on the directory, *t2_reservoir_gate_transfer*, and
        select *Open Command Window Here*.<BR>
...
1.  **Brain teasers**
    1.  The equation for inflow from a node to a reservoir through a
        ...
        coefficient C lump together?<BR>
