...
// TODO
// * Add __doc__ to unfuncs, PyMultiarray and umath.
...
//   Math.pow. (double and float cases would use math.pow).
...
public class PyMultiarray extends PySequence {
	...
	*/ 
	...
	public static PyMultiarray fromFunction(PyFunction function, PyObject shape) {
	...
	public static PyMultiarray fromString(String s, char type) throws java.io.IOException {
		...
		Object data = fromByteArray(s.getBytes(), type);
		return new PyMultiarray(data, type, 0, new int [] {Array.getLength(data)}, new int [] {1});
	}
	...
	public static PyMultiarray asarray(PyObject seq, char typecode) {
		if (seq instanceof PyMultiarray && ((PyMultiarray)seq)._typecode == typecode) 
	...
	public PyMultiarray astype(char type) {return array(this, type);}

	/** Return multiarray data represented as a string.*/
	public String tostring() throws java.io.IOException {
		return new String(toByteArray(array(this).data, _typecode));
	}
	
	/** Return a multiarray with data byte swapped (little to big endian).*/
	public final PyMultiarray byteswapped() throws java.io.IOException {
	...
	public static void set_string_function(PyFunction pf, int which) {
		str_functions[which] = pf;
	}
	...
	public static PyMultiarray take(PyObject oA, PyObject oIndices, int axis) {
		...
		if (axis < 0 || axis >= a.dimensions.length)
			throw Py.ValueError("illegal axis");
		a = rotateAxes(ascontiguous(a), -axis);
		...
		for (int i = 0; i < indices.dimensions[0]; i++)
			result.set(i, a.get(Array.getInt(indices.data, i)));
		...
		for (int i = 0; i < values.dimensions[0]; i++) {
			...
			if (a._typecode != 'O' && values._typecode != 'O') {
				...
				while (start != stop) {
					...
					if (val._eq(value).__nonzero__()) break;
					if (val._ge(value).__nonzero__()) stop = j;
					if (val._le(value).__nonzero__()) start = j;
					j = (start+stop)/2;
				}
			}
	...
	private final static int _seqToObjects(PyObject seq, PyObject [] flat, int offset) {
		...
		for (int i = 0; i < items; i++)
			delta += _seqToObjects(seq.__getitem__(i), flat, offset+delta);
		return delta;
	}
	...
	private final static Object objectToJava(PyObject o, char typecode, boolean returnReal) {
		...
		if (o instanceof PyComplex)
			...
			o = Py.Zero;
		Number number = (Number)o.__tojava__(typeToClass(typecode));
	...
	public Object __tojava__(Class c) {
		...
		if (dimensions.length == 1 || _typecode == 'F' || _typecode == 'D')
			return super.__tojava__(c); // Punt!
		if (c == Object.class || (c.isArray() && c.getComponentType().isAssignableFrom(type))) {
			Class component = c.getComponentType();
			Object o = Array.newInstance(component, dimensions[0]);
			for (int i = 0; i < dimensions[0]; i++)
				Array.set(o, i, get(i).__tojava__(component));
			return o;
		}
	...
	protected void setslice(int start, int stop, int step, PyObject value) {
		__setitem__(new PySlice(Py.newInteger(start), Py.newInteger(stop), Py.newInteger(step)), value);
	}

	// XXX Optimize!!
	protected PyObject getslice(int start, int stop, int step) {
		return __getitem__(new PySlice(Py.newInteger(start), Py.newInteger(stop), Py.newInteger(step)));
	}

	/** Return the repr for this Multiarray, default to toString if provided function fails.*/
	public PyString __repr__() {
		try {return (PyString) str_functions[1].__call__(this);}
		catch (Throwable t) {return new PyString(toString());}
	}

	/** Return the str for this Multiarray, default to toString if provided function fails.*/
	public PyString __str__() {
		try {return (PyString) str_functions[0].__call__(this);}
		catch (Throwable t) {return new PyString(toString());}
	}
	...
	/** Convert a negative or None slice index to a positive index.*/
	protected int fixSliceIndex(PyObject pyIndex, int axis, boolean max) {
		if (pyIndex instanceof PyNone) 
			return max ? dimensions[axis] : 0;
		int index = ((PyInteger)pyIndex).getValue();
		if (index < 0) index += dimensions[axis];
		if (index < 0) return 0;
		...
		for (int i = 0; i < indices.length; i++) {
			...
			if (index instanceof PyEllipsis) {
				...
				if (!(slice.step instanceof PyNone)) 
					sliceStep = ((PyInteger)slice.step).getValue();
				int sliceStart = fixSliceIndex(slice.start, oldAxis, !(sliceStep > 0));
				int sliceStop = fixSliceIndex(slice.stop, oldAxis, (sliceStep > 0));
				if (sliceStep > 0) {
					...
					newStart += (sliceStart-1) * strides[oldAxis];
				}
	...
	static byte [] toByteArray(Object data, char type) throws java.io.IOException {
	...
	static Object fromByteArray(byte [] bytes, char type) throws java.io.IOException {
...
// Begin generated code (genOps).
	...
	private final static void powShort(int sa, PyMultiarray a, int sb, PyMultiarray b, int sr, PyMultiarray r, int d) {
		...
		if (d == r.dimensions.length - 1) {
			...
			final short[] rData = (short[])r.data;
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
				rData[sr] = (short)(java.lang.Math.pow(aData[sa], bData[sb]));
		}
		else	
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
	...
	private final static void powInt(int sa, PyMultiarray a, int sb, PyMultiarray b, int sr, PyMultiarray r, int d) {
		...
		if (d == r.dimensions.length - 1) {
			...
			final int[] rData = (int[])r.data;
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
				rData[sr] = (int)(java.lang.Math.pow(aData[sa], bData[sb]));
		}
		else	
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
	...
	private final static void powLong(int sa, PyMultiarray a, int sb, PyMultiarray b, int sr, PyMultiarray r, int d) {
		...
		if (d == r.dimensions.length - 1) {
			...
			final long[] rData = (long[])r.data;
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
				rData[sr] = (long)(java.lang.Math.pow(aData[sa], bData[sb]));
		}
		else	
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
	...
	private final static void powFloat(int sa, PyMultiarray a, int sb, PyMultiarray b, int sr, PyMultiarray r, int d) {
		...
		if (d == r.dimensions.length - 1) {
			...
			final float[] rData = (float[])r.data;
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
				rData[sr] = (float)(java.lang.Math.pow(aData[sa], bData[sb]));
		}
		else	
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
	...
	private final static void powDouble(int sa, PyMultiarray a, int sb, PyMultiarray b, int sr, PyMultiarray r, int d) {
		...
		if (d == r.dimensions.length - 1) {
			...
			final double[] rData = (double[])r.data;
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
				rData[sr] = (double)(java.lang.Math.pow(aData[sa], bData[sb]));
		}
		else	
			for (; sr != maxSr; sa+=dsa, sb+=dsb, sr+=dsr)
