...
On Error Resume Next
  Dim fso, folder, files, NewsFile, docFolder, pdfFolder, docPath, pdfPath
 
  Set fso = CreateObject("Scripting.FileSystemObject")
  docFolder = "..\tutorials\doc\"
  pdfFolder = "..\tutorials\pdf\"
  If docFolder = "" Then
      Wscript.Echo "No Folder parameter was passed"
      Wscript.Quit
  End If
  'Set NewFile = fso.CreateTextFile(docFolder&"FileList.txt", True)
  Set folder = fso.GetFolder(docFolder)
  Set files = folder.Files
 
  For each folderIdx In files
    if instr(folderIdx, "icons.doc") = 0 then
    docPath =  docFolder & folderIdx.Name

    pdfOldPath = docFolder & Replace(folderIdx.Name,".doc",".pdf")
    pdfNewPath = pdfFolder & Replace(folderIdx.Name,".doc",".pdf")

    Call DOC2PDF_sub( docPath, "")
    
    Set fso2 = CreateObject("Scripting.FileSystemObject")
    Set aFile = fso2.GetFile(pdfOldPath)
    aFile.Move pdfNewPath
...
Function DOC2PDF_sub( sDocFile, sPDFFile )
