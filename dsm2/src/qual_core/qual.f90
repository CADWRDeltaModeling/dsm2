module dsm2qual
    ...
    use common_qual
    use common_tide
    use common, only: update_intervals
    use network
    use dss
    use mod_readdss
    use mod_writedss
    ...
    use qual_get_output
    use mod_masstrack
    use qual_rate_chanres
    use qual_restart_file
    use qual_interpx
    use qual_boundary
    use qual_heat
    use qual_route
    use kinetic_qual
    use mod_buffer_input_tidefile
    use mod_buffer_input_grid
    use mod_check_fixed
    use reservoir_geometry
    use mod_store_outpaths
    use hdf_tidefile
    use qual_process_tide
...
!-----(NOPR should be at least 20 + 2 times NOSC)
    ...
    ! Add submodule procedures
    interface
        module subroutine qual_init()
        end subroutine qual_init

        module subroutine bltminit()
        end subroutine bltminit

        module subroutine print_outqual(istat)
            integer :: istat
        end subroutine print_outqual
...
subroutine qual_prepare2()
    use mod_fixed
    use mod_masstrack
    use mod_buffer_input_qual
...
subroutine qual_check_input_data()
    implicit none
...
!-----------Read the Hydro tidefile
        call read_mult_tide(process_tide)
...
subroutine qual_wrapup()
    call qual_wrapup1()
    call qual_wrapup2()
end subroutine

subroutine qual_wrapup1()
    implicit none
...
subroutine qual_prepare_loop()
    implicit none

    call update_intervals

    call init_store_outpaths(istat, get_output_qual)
...
end subroutine qual_prepare_loop

subroutine qual_loop()
    use hdf_tidefile, only: read_mult_tide
    implicit none
...
!                        ENDDO
            ...
            end if ! if not stage-boundary node

640     END DO !  DO 640 JN=1,NNODES
...
910             format(I5, ' nodes cannot be mixed;', / &
    ...
    END DO ! do while (.not.AllJunctionsMixed)
