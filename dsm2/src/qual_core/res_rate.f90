...
!-----Return flows and massrates, either into or out of, for qual reservoir,
!-----for given group label index (if none given ignore group
!-----labels).
      Use IO_Units
      Use groups_data
      use common_tide
...
!--------------does group label match?
      ...
      if (group_ndx_ok) then
!--------reservoir flows connected to nodes
...
!--------------------internal node or external non-stage-BC nodes connected to a reservoir
                     conc=node_qual(intnode,j)
                     massrate(j)=massrate(j) &
                         - qres(res,i) * conc
                  else
!--------------------external node (stage BC) connected to a reservoir
                     if (listup(intnode,1).gt.0) then
!-----------------------reservoir connected at the upstream boundary
                        ich=listup(intnode,1)
                        massrate(j)=massrate(j)+gptu(j,ich)/(dt*3600.)
                     else if (listdown(intnode,1).gt.0) then
!-----------------------reservoir connected at the downstream boundary
