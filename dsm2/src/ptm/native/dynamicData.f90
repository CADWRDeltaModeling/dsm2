...
!-------- flow into node +ve and flow out of node -ve
!-------- channel flow +ve from up node to down node
...
!----------- flow outof reservoir is +ve, thus towards node thus +ve
      !todo: eli changed from qresv
            iconnect = iconnect + 1
            wb(id).flowToNode(k) = qresv(iconnect)
         enddo
!-------- update internal flows ( assumption of order important: fixedData.f)
         j=1
         k = res_geom(i).nnodes
         do while(res_geom(i).qinternal(j) .ne. 0)
            ...
            j = j + 1
         enddo
!-------- update external flows ( assumption of order important: fixedData.f)
         j=1
...
!-----get_diversion_at_node= qNodeDiversion(number)
      return
      end
...
!-----get_reservoir_pumping= qReservoirPumping(number)
      return
      end
!-----++++++++++++++++++++++++++++++++++++++++++++++++++
...
!     UpdateTimeVaryingData and get all filter ops for the specified timestamp
      use grid_data
      use type_defs
      use iopath_data
      use runtime_data
...
!     get the filter op for the specified filter at current timestamp
      use common_ptm
      implicit none
      integer number
