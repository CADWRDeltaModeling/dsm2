...
public class PTMEnv{
	...
	public PTMEnv(){
		//Input files
		fixedInput = new PTMFixedInput();
		hydroInput = new PTMHydroInput(fixedInput);
	...
	private final void setWaterbodyInfo(){
		...
		//wbArray starts from 1. see PTMFixedInput.java
		...
		for (int i=1; i<=fixedInput.getMaximumNumberOfWaterbodies(); i++){
			if(wbArray[i] != null) {
				...
				for(int j=0; j< nNodes; j++){
					// the node id array has already been created in PTMFixedInput, here to create a node array
	...
	public void addBehaviors(){
		...
		_behaviorInputs = new PTMBehaviorInputs(nodeArray, wbArray);
  ...
  void updateBoundaryWaterbodiesHydroInfo(){
	 ...
	 *  reservoirs have Node info however the Node as defined by tide file
