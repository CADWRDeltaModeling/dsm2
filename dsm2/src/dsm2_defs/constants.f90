...
module constants
    !! Module for common constants used throughout DSM2

    use iso_c_binding
    implicit none

    ! Kind parameters for C interoperability
    ...
    ! NOTE: Not sure what the variable comment means above.
    ...
    ! ----magic numbers for missing values and initialization
    integer, parameter :: &
        initial_channel_length = -99999999, &
        chan_mid = 88888, &
        chan_up = 77771, &
        chan_down = 77770, &
        miss_val_i = -901, &
    ...
    ! Constants primarily from GTM
    ! Properly-typed real constants
    real(kind=DP), parameter :: MINUS = -1.d0
    ...
    real(kind=DP), parameter :: THREE = 3.d0
        !! Real constant 3.   properly typed
    real(kind=DP), parameter :: FOUR = 4.d0
        !! Real constant 4.   properly typed
    real(kind=DP), parameter :: FIVE = 5.d0
        !! Real constant 5.   properly typed
    real(kind=DP), parameter :: SIX = 6.d0
        !! Real constant 6.   properly typed
    real(kind=DP), parameter :: SEVEN = 7.d0
        !! Real constant 7.   properly typed
    real(kind=DP), parameter :: EIGHT = 8.d0
        !! Real constant 8.   properly typed
    real(kind=DP), parameter :: NINE = 9.d0
        !! Real constant 9.   properly typed
    real(kind=DP), parameter :: TEN = 1.d1
        !! Real constant 10.  properly typed
    real(kind=DP), parameter :: HALF = 5.d-1
        !! Real constant 0.5  properly typed
    real(kind=DP), parameter :: THIRD = ONE / THREE
        !! Real constant 1/3  properly typed
    real(kind=DP), parameter :: TWENTYFOUR = 2.4d1
        !! Real constant 24.  properly typed
    real(kind=DP), parameter :: SIXTY = 6.d1
        !! Real constant 60.  properly typed
    real(kind=DP), parameter :: MILLION = 1.0d6
        !! Real constant 1,000,000.  properly typed

    integer, parameter :: LARGEINT = -123456789
    real(kind=DP), parameter :: LARGEREAL = 1.23456789d8
        !! Absurd high value, for initialization and for marking undefined
        !! data in calculations. This makes bugs easier to spot.
    real(kind=DP), parameter :: PI = acos(-ONE)
        !! Pi constant
    real(kind=DP), parameter :: EPS = 1.d-15
        !! Default epsilon for testing closeness
    real(kind=DP), parameter :: WEAK_EPS = 1.d-9
        !! Weaker epsilon for testing approximate equality or algorithms
    real(kind=DP), parameter :: WEAKEST_EPS = 1.d-3
        !! Weakest epsilon for testing approximate equality
    real(kind=DP), parameter :: SMALL = 1.00e-6
