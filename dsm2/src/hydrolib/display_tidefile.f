program display_tidefile
    ...
    !-----display values in a hydro tidefile
    ...
    !-----get tidefile name from command line arg
    call getarg(One,tidefile)
    if (lnblnk(tidefile) == 0) then ! no command line arg
...
!-----print time-varying info for each tideblock
100 continue
    print *
    read(unit,end=200) TideTime ! time at end of tide averaged block
    !-----instaneous values at start of tide block
    ...
    !-----the averages are between this TideTime and previous TideTime
    ...
    do i=1,max_reservoirs
        ...
        enddo
        j=1
        do while (res_geom(i)%qext(j) /= 0)
            extndx=res_geom(i)%qext(j)
            res_net_flow(i)=res_net_flow(i)+qext(extndx)%avg
            j=j+1
        enddo
        j=1
        do while (res_geom(i)%qint(j) /= 0)
            intndx=res_geom(i)%qint(j)
            if (obj2obj(intndx)%from%object == obj_reservoir .and. &
                obj2obj(intndx)%from%object_no == i) then
                res_net_flow(i)=res_net_flow(i)-obj2obj(intndx)%flow_avg
            endif
            if (obj2obj(intndx)%to%object == obj_reservoir .and. &
                obj2obj(intndx)%to%object_no == i) then
                res_net_flow(i)=res_net_flow(i)+obj2obj(intndx)%flow_avg
            endif
            j=j+1
        enddo
    ...
    do i=1,max_reservoirs
        if (qresv(i,1) /= 0.0 .and. qresv(i,2) /= 0.0) &
    ...
    do i=1,max_reservoirs
        if (eresv(i) /= 0.0) print *, i,eresv(i),res_net_flow(i)
    enddo
