...
c--------indices must be cumulative:  add current indices to indices for next chan
     ...
     &        xsect_assg(channo).num_sec_assg .le. 0 ) then
...
c-----check array bounds
      if(xsect_assg(channo).num_sec_assg .gt. max_assg_sec) then
...
c-----assign elev for rectangular section
      if (xsect_assg(channo).num_sec_assg .eq. 0) then
         temp_all_elev(k)=0.0
         k=k+1
...
c-----assign rectangular xsect if none available for interpolation at chan. ends
      if (xsect_assg(channo).num_sec_assg .eq. 0) then
		write(unit_error,*) 'No cross-sections available for channel: ',
...
c-----downstream end
      ...
      if (xsect_assg(channo).rect(xsect_assg(channo).num_sec_assg)) then
         ...
         if ( xsect_assg(channo).dist(num_sec) .lt. 
     &        chan_geom(channo).length) then
            xs=chan_geom(channo).xsect(2)
            virt_min_elev(minelev_index(channo)+vsecno-1) = xsect_geom(xs).botelv
         elseif(xsect_assg(channo).dist(num_sec) .ge.
     &           chan_geom(channo).length) then
...
c-----interpolate interior virtual xsect bottom elevations
      do vsecno=2,num_virt_sec(channo)-1
         ...
         downindex=0
         do i=1,num_sec
            if ( xsect_assg(channo).dist(i) .le. x .and. 
     &           .not. xsect_assg(channo).rect(i) ) upindex=i
         enddo
         do i=num_sec,1,-1
            if ( xsect_assg(channo).dist(i) .ge. x .and. 
     &           .not. xsect_assg(channo).rect(i) ) downindex=i
         enddo
c--------if there is a xsect with dist=x, then both indices will be the same
     ...
     &           irreg_geom(xsect_assg(channo).sec_index(upindex)).min_elev
         else
...
c-----------find upindex, downindex,x1,x2,y1-8 
            ...
            downindex=0
            do i=1,num_sec
               if (xsect_assg(channo).dist(i) .le. x) upindex=i
            enddo
            do i=num_sec,1,-1
               if (xsect_assg(channo).dist(i) .ge. x) downindex=i
            enddo
...
c-----statement function to calculate index of minimum elevation array
     ...
     &        (xsect_assg(channo).num_sec_assg .gt. 0) ) then
            do vsecno=1,num_virt_sec(channo)
