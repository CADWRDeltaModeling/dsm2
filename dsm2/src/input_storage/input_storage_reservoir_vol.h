#ifndef reservoir_vol_STORAGE_H__
#define reservoir_vol_STORAGE_H__
/**
...
/** Structure representing input of type reservoir_vol.
...
public:
  ...
  /** Data type reservoir_vol, default constructor */  
  typedef const boost::tuple<const std::string,const double&>  identifier_type;
  ...
  /** Construct a reservoir_vol with actual data values */
  reservoir_vol(const  char a_res_name[32],const double & a_elev,const double & a_area,const double & a_volume, bool a_used=true, int a_layer = 0) :
    elev(a_elev),
    area(a_area),
    volume(a_volume),
    used(a_used),
  ...
  /**Copy constructor) 
   */
  reservoir_vol (const reservoir_vol & other) :
    elev(other.elev),
    area(other.area),
    volume(other.volume),
    used(other.used),
  ...
  */
  ...
  bool operator< (const reservoir_vol & other) const
  {
  ...
  reservoir_vol& operator=(const reservoir_vol& rhs)
  {
  ...
  /** Return the class name of this object (reservoir_vol) */
  string objectName() const
  { 
    return "reservoir_vol"; 
  }
...
typedef HDFTableManager<reservoir_vol> reservoir_vol_table;
...
*/  
FCALL void reservoir_vol_clear_buffer_f();

/** query number of records being stored in buffer */
FCALL int reservoir_vol_buffer_size_f();


/** append to buffer, compatible with fortran, returns new size*/
FCALL void reservoir_vol_append_to_buffer_f(const  char a_res_name[32],const double * a_elev,const double * a_area,const double * a_volume, int * ierror, 
              const int res_name_len);
  
/** both makes the table and writes the contents of the buffer to it */
FCALL void reservoir_vol_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror);

/** reads the table in from a file into the buffer*/
FCALL void reservoir_vol_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror);

/** query size information about the table in hdf5
*/
FCALL void reservoir_vol_number_rows_hdf5_f(const hid_t* file_id, hsize_t* nrecords, int* ierror);


/** get one row worth of information from the buffer */
FCALL void reservoir_vol_query_from_buffer_f(size_t* row, 
                         char a_res_name[32],double * a_elev,double * a_area,double * a_volume, int * ierror, 
              int res_name_len);
/**
  prioritize buffer by layers, delete unused items and sort
  */
FCALL void reservoir_vol_prioritize_buffer_f(int* ierror);
/**
 ...
 */
FCALL void reservoir_vol_write_buffer_to_text_f(const char* file, const bool* append, int* ierror, int filelen);
