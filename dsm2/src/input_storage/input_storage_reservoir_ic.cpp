...
/**
  ...
  4. Clear the buffer.
*/
#include "input_storage_reservoir_ic.h"
...
template<>
HDFTableManager<reservoir_ic>::HDFTableManager() :
    description(reservoir_ic_table_description()),  
    m_default_fill(reservoir_ic("",-901.0)){}

template<>
void HDFTableManager<reservoir_ic>::prioritize_buffer()
{

    // Sort by identifier (lexicographical order) and
    // layer (decreasing order of priority)
    std::sort(buffer().begin(),buffer().end());
    vector<reservoir_ic>::const_iterator dupl = adjacent_find(buffer().begin(),buffer().end());
    if ( dupl != buffer().end())
    {   
    ...
    buffer().erase(unique(buffer().begin(),buffer().end(),identifier_equal<reservoir_ic>()),buffer().end());
    // Eliminate items that are not used. This must be done after lower layers have been removed
    buffer().erase(remove_if(buffer().begin(), buffer().end(),not1(entry_used<reservoir_ic>())), buffer().end());
    
}

TableDescription reservoir_ic_table_description(){
  const char* title = "reservoir_ic";
  const size_t size = sizeof(reservoir_ic);
  const size_t nfields = 2;
  reservoir_ic default_struct = reservoir_ic("",-901.0);
  const char* fnames[] =  {"res_name","stage"};
  const hid_t ftypes[] =  {
            string_type(32),H5T_NATIVE_DOUBLE
               };
...
/**
  Clear the storage buffer for objects of type reservoir_ic
*/  
void reservoir_ic_clear_buffer_f(){
  //reservoir_ic_table::instance().buffer().destroy();
  reservoir_ic_table::instance().buffer().clear();
}

/** append to buffer, compatible with fortran, returns new size*/
void reservoir_ic_append_to_buffer_f(const  char a_res_name[32],const double * a_stage, int * ierror, 
              const int res_name_len)
{
 _TRAP_EXCEPT(*ierror,
   reservoir_ic_table::instance().buffer().push_back(
                                      reservoir_ic(
                                      a_res_name,*a_stage
                                      ));
...
void reservoir_ic_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  reservoir_ic_table & table = reservoir_ic_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( reservoir_ic_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //reservoir_ic_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void reservoir_ic_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    reservoir_ic_table & table = reservoir_ic_table::instance();
...
void reservoir_ic_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     reservoir_ic_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
void reservoir_ic_query_from_buffer_f(size_t* row, 
                         char a_res_name[32],double * a_stage, int * ierror, 
              int res_name_len
                        )
{
 _TRAP_EXCEPT(*ierror,
    //if (row > reservoir_ic_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    reservoir_ic obj =reservoir_ic_table::instance().buffer()[ndx];
...
void reservoir_ic_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  reservoir_ic_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type reservoir_ic */
int reservoir_ic_buffer_size_f()
{ 
  return (int) reservoir_ic_table::instance().buffer().size();
}

void reservoir_ic_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("reservoir_ic");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<reservoir_ic> & obs = reservoir_ic_table::instance().buffer();
   reservoir_ic_table& table = reservoir_ic_table::instance();
   ...
   out << endl;
   for (vector<reservoir_ic>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const reservoir_ic & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void reservoir_ic_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  reservoir_ic_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
