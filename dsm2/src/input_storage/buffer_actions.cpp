...
HDFTableManager<reservoir_vol>::instance().buffer().clear();
...
HDFTableManager<reservoir>::instance().prioritize_buffer();
     if(*ierror != 0) return;
HDFTableManager<reservoir_vol>::instance().prioritize_buffer();
     if(*ierror != 0) return;
...
reservoir_write_buffer_to_text_f(file,append,ierror,filelen);
     if(*ierror != 0) return;
reservoir_vol_write_buffer_to_text_f(file,append,ierror,filelen);
     if(*ierror != 0) return;
...
if(buffer_name == "reservoir"){reservoir_write_buffer_to_text_f(file,append,ierror,filelen);
     if(*ierror != 0) return;}
if(buffer_name == "reservoir_vol"){reservoir_vol_write_buffer_to_text_f(file,append,ierror,filelen);
     if(*ierror != 0) return;}
...
if(buffer_name == "reservoir"){reservoir_write_buffer_to_hdf5_f(file_id,ierror);
     if(*ierror != 0) return;}
if(buffer_name == "reservoir_vol"){reservoir_vol_write_buffer_to_hdf5_f(file_id,ierror);
     if(*ierror != 0) return;}
...
if(buffer_name == "reservoir"){reservoir_read_buffer_from_hdf5_f(file_id,ierror);
     if(*ierror != 0) return;}
if(buffer_name == "reservoir_vol"){reservoir_vol_read_buffer_from_hdf5_f(file_id,ierror);
     if(*ierror != 0) return;}
...
reservoir_write_buffer_to_hdf5_f(file_id,ierror);
     if(*ierror != 0) return;
reservoir_vol_write_buffer_to_hdf5_f(file_id,ierror);
     if(*ierror != 0) return;
