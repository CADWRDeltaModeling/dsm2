...
/**
  ...
  4. Clear the buffer.
*/
#include "input_storage_boundary_flow.h"
...
template<>
HDFTableManager<boundary_flow>::HDFTableManager() :
    description(boundary_flow_table_description()),  
    m_default_fill(boundary_flow("",-901,-901,"","","")){}

template<>
void HDFTableManager<boundary_flow>::prioritize_buffer()
{

    // Sort by identifier (lexicographical order) and
    // layer (decreasing order of priority)
    std::sort(buffer().begin(),buffer().end());
    vector<boundary_flow>::const_iterator dupl = adjacent_find(buffer().begin(),buffer().end());
    if ( dupl != buffer().end())
    {   
    ...
    buffer().erase(unique(buffer().begin(),buffer().end(),identifier_equal<boundary_flow>()),buffer().end());
    // Eliminate items that are not used. This must be done after lower layers have been removed
    buffer().erase(remove_if(buffer().begin(), buffer().end(),not1(entry_used<boundary_flow>())), buffer().end());
    
}

TableDescription boundary_flow_table_description(){
  const char* title = "boundary_flow";
  const size_t size = sizeof(boundary_flow);
  const size_t nfields = 6;
  boundary_flow default_struct = boundary_flow("",-901,-901,"","","");
  const char* fnames[] =  {"name","node","sign","fillin","file","path"};
  const hid_t ftypes[] =  {
            string_type(32),H5T_NATIVE_INT,H5T_NATIVE_INT,string_type(8),string_type(128),string_type(80)
               };
...
/**
  Clear the storage buffer for objects of type boundary_flow
*/  
void boundary_flow_clear_buffer_f(){
  //boundary_flow_table::instance().buffer().destroy();
  boundary_flow_table::instance().buffer().clear();
}

/** append to buffer, compatible with fortran, returns new size*/
void boundary_flow_append_to_buffer_f(const  char a_name[32],const int * a_node,const int * a_sign,const  char a_fillin[8],const  char a_file[128],const  char a_path[80], int * ierror, 
              const int name_len,const int fillin_len,const int file_len,const int path_len)
{
 _TRAP_EXCEPT(*ierror,
   boundary_flow_table::instance().buffer().push_back(
                                      boundary_flow(
                                      a_name,*a_node,*a_sign,a_fillin,a_file,a_path
                                      ));
 ) // end of exception trap
}
  
/** both makes the table and writes the contents of the buffer to it */
void boundary_flow_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
  boundary_flow_table & table = boundary_flow_table::instance();
    *ierror = static_cast<int>( H5TBmake_table( boundary_flow_table::instance().description.title.c_str(), 
                                              *file_id, 
		                                     ...
		                                     &table.default_fill(), //fill data 
		                                       1,                     //boundary_flow_table::instance().description.compress, 
		                                      table.buffer().size() > 0 ? &table.buffer()[0] : NULL));
  ) // end of exception trap
}

/** reads the table in from a file into the buffer*/
void boundary_flow_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields;
    hsize_t nrecords;
    boundary_flow_table & table = boundary_flow_table::instance();
...
void boundary_flow_number_rows_hdf5_f(const hid_t *file_id, hsize_t* nrecords, int* ierror){
 _TRAP_EXCEPT(*ierror,
    hsize_t nfields = 0;
    *ierror = static_cast<int>(  H5TBget_table_info (*file_id, 
				     boundary_flow_table::instance().description.title.c_str(), 
				     &nfields, 
				     nrecords));
 ) // end of exception trap
}
...
void boundary_flow_query_from_buffer_f(size_t* row, 
...
{
 _TRAP_EXCEPT(*ierror,
    //if (row > boundary_flow_table::instance().buffer().size()) return -2; //todo: HDF_STORAGE_ERROR;
    size_t ndx = *row - 1;
    boundary_flow obj =boundary_flow_table::instance().buffer()[ndx];
...
void boundary_flow_prioritize_buffer_f(int* ierror)
{  
 _TRAP_EXCEPT(*ierror,
  boundary_flow_table::instance().prioritize_buffer();
   ) // end of exception trap
}

/** Query the size of the storage buffer for objects of type boundary_flow */
int boundary_flow_buffer_size_f()
{ 
  return (int) boundary_flow_table::instance().buffer().size();
}

void boundary_flow_write_buffer_to_stream(ostream & out, const bool& append)
{
   string keyword("boundary_flow");
   boost::to_upper(keyword);
   out << keyword <<endl;
   vector<boundary_flow> & obs = boundary_flow_table::instance().buffer();
   boundary_flow_table& table = boundary_flow_table::instance();
   ...
   out << endl;
   for (vector<boundary_flow>::const_iterator it = obs.begin();
        it != obs.end(); ++it)
        {  
           const boundary_flow & outitem = *it;
           out << outitem << endl;
        }
   out << "END\n" << endl;
}

void boundary_flow_write_buffer_to_text_f(const char* file, 
                                      const bool* append, 
                                      int* ierror, 
                                      int filelen)
{
 _TRAP_EXCEPT(*ierror,
  ...
  boundary_flow_write_buffer_to_stream(out,*append); 
  ) // end of exception trap  
}
