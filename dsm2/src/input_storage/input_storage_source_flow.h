#ifndef source_flow_STORAGE_H__
#define source_flow_STORAGE_H__
/**
...
/** Structure representing input of type source_flow.
...
public:
  ...
  /** Data type source_flow, default constructor */  
  ...
  /** Construct a source_flow with actual data values */
  source_flow(const  char a_name[32],const int & a_node,const int & a_sign,const  char a_fillin[8],const  char a_file[128],const  char a_path[80], bool a_used=true, int a_layer = 0) :
    node(a_node),
    sign(a_sign),
    used(a_used),
    layer(a_layer)
  ...
  /**Copy constructor) 
   */
  source_flow (const source_flow & other) :
    node(other.node),
    sign(other.sign),
    used(other.used),
    layer(other.layer)
  ...
  /** Parent object class name.
      ...
      of this class.
  */
  source_flow::identifier_type parent_identifier()  const
  {
     return source_flow::identifier_type( name );
  }

  /** Return the version/layer number of the parent object */ 
  int parent_version()  const
  {
    vector<source_flow>& pbuf = HDFTableManager<source_flow>::instance().buffer();
    source_flow parent;
    parent.set_identifier(parent_identifier());
    vector<source_flow>::iterator loc = lower_bound(pbuf.begin(),
                                                pbuf.end(),
                                                parent,
                                                identifier_compare<source_flow>());
  ...
  bool operator< (const source_flow & other) const
  {
  ...
  source_flow& operator=(const source_flow& rhs)
  {
  ...
  /** Return the class name of this object (source_flow) */
...
typedef HDFTableManager<source_flow> source_flow_table;
...
*/  
FCALL void source_flow_clear_buffer_f();

/** query number of records being stored in buffer */
FCALL int source_flow_buffer_size_f();


/** append to buffer, compatible with fortran, returns new size*/
FCALL void source_flow_append_to_buffer_f(const  char a_name[32],const int * a_node,const int * a_sign,const  char a_fillin[8],const  char a_file[128],const  char a_path[80], int * ierror, 
              const int name_len,const int fillin_len,const int file_len,const int path_len);
  
/** both makes the table and writes the contents of the buffer to it */
FCALL void source_flow_write_buffer_to_hdf5_f(const hid_t* file_id, int* ierror);

/** reads the table in from a file into the buffer*/
FCALL void source_flow_read_buffer_from_hdf5_f(const hid_t* file_id, int* ierror);

/** query size information about the table in hdf5
*/
FCALL void source_flow_number_rows_hdf5_f(const hid_t* file_id, hsize_t* nrecords, int* ierror);


/** get one row worth of information from the buffer */
FCALL void source_flow_query_from_buffer_f(size_t* row, 
...
/**
  ...
  */
FCALL void source_flow_prioritize_buffer_f(int* ierror);
/**
 ...
 */
FCALL void source_flow_write_buffer_to_text_f(const char* file, const bool* append, int* ierror, int filelen);
