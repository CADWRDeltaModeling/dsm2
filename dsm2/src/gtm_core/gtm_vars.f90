...
!****************************************************************************************************************************************
    ...
    real (gtm_real) :: Henry_Hg0 = 0.29           !> Henry's Law constant for elemental Hg
    ...
    real (gtm_real) :: Henry_MeHg = 1.6d-05  !> Henry's Law constant for MeHg
    ...
    real (gtm_real) :: mtc_sed_wat          !> Mass transfer coefficient for Hg diffusion between water and sediment compartments (m/d)
     ...
     subroutine allocate_group
         ...
         coeff_name(2) = "decay"
         coeff_name(3) = "settle"
         coeff_name(4) = "benthic"
         coeff_name(5) = "alg_grow"
         coeff_name(6) = "alg_resp"
         coeff_name(7) = "alg_die"
         return
     end subroutine
     ...
     subroutine check_group_channel(ncc_code,      &
         ...
         do i = 1, n_chan
             if (group_var_chan(ncc_code,group_var_code,i).eq.LARGEREAL) then
                 ...
                 write(unit_error,'(a31,2a32,a16,i10)') "You forgot to assign values for ",ncc_name,coeff_name(group_var_code)," for Channel No.",chan_geom(i)%channel_num
             end if
         end do
         return
     end subroutine
